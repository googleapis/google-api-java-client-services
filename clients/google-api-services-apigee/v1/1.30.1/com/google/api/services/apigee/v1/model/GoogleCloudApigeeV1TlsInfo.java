...
public final class GoogleCloudApigeeV1TlsInfo extends com.google.api.client.json.GenericJson {
   ...
   * target endpoints, and when configuring virtual hosts that use 2-way TLS.
   *
   * When used with a target endpoint/target server, if the backend system uses SNI and returns a
   * cert with a subject Distinguished Name (DN) that does not match the hostname, there is no way
