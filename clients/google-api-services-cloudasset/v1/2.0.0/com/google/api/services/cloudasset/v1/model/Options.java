...
public final class Options extends com.google.api.client.json.GenericJson {
   ...
   * to a Google Cloud folder F, then user A potentially has access to the Google Cloud folder F.
   * And those advanced analysis results will be included in
   * AnalyzeIamPolicyResponse.service_account_impersonation_analysis. Another example, if the
   * request analyzes for who has permission P to a Google Cloud folder F, and there's an IAM policy
   * states user A has iam.serviceAccounts.actAs permission to a service account SA, and there's
   * another IAM policy states service account SA has permission P to the Google Cloud folder F,
   * then user A potentially has access to the Google Cloud folder F. And those advanced analysis
   * results will be included in AnalyzeIamPolicyResponse.service_account_impersonation_analysis.
   * Only the following permissions are considered in this analysis: * `iam.serviceAccounts.actAs` *
   ...
   * A has permission P, and the results include an IAM policy with P on a Google Cloud folder, the
   * results will also include resources in that folder with permission P. If true and
   ...
   * project or lower resources are supported. Folder and organization resources cannot be used
   * together with this option. For example, if the request analyzes for which users have permission
   * P on a Google Cloud project with this option enabled, the results will include all users who
   * have permission P on that project or any lower resource. If true, the default max expansion per
