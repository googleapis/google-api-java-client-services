...
/**
 * Singleton resource representing the set of Slow Start metrics. This metric set contains Activity
 * start duration data. **Supported aggregation periods:** * DAILY: metrics are aggregated in
 * calendar date intervals. Due to historical constraints, the only supported timezone is
 * `America/Los_Angeles`. **Supported metrics:** * `slowStartRate` (`google.type.Decimal`):
 ...
 * `slowStartRate28dUserWeighted` (`google.type.Decimal`): Rolling average value of `slowStartRate`
 * in the last 28 days. The daily values are weighted by the count of distinct users for the day. *
 * `distinctUsers` (`google.type.Decimal`): Count of distinct users in the aggregation period that
 * were used as normalization value for the `slowStartRate` metric. A user is counted in this metric
 * if their app was launched in the device. Care must be taken not to aggregate this count further,
 * as it may result in users being counted multiple times. The value is rounded to the nearest
 * multiple of 10, 100, 1,000 or 1,000,000, depending on the magnitude of the value. **Required
 * dimension:** This dimension must be specified with each request for the request to be valid. *
 * `startType` (string): the type of start that was measured. Valid types are `HOT`, `WARM` and
 * `COLD`. **Supported dimensions:** * `apiLevel` (string): the API level of Android that was
 * running on the user's device, e.g., 26. * `versionCode` (int64): version of the app that was
 * running on the user's device. * `deviceModel` (string): unique identifier of the user's device
 * model. The form of the identifier is 'deviceBrand/device', where deviceBrand corresponds to
 * Build.BRAND and device corresponds to Build.DEVICE, e.g., google/coral. * `deviceBrand` (string):
 * unique identifier of the user's device brand, e.g., google. * `deviceType` (string): the type
...
public final class GooglePlayDeveloperReportingV1alpha1SlowStartRateMetricSet extends com.google.api.client.json.GenericJson {
   ...
   */
  public GooglePlayDeveloperReportingV1alpha1SlowStartRateMetricSet setFreshnessInfo(GooglePlayDeveloperReportingV1alpha1FreshnessInfo freshnessInfo) {
   ...
   */
  public GooglePlayDeveloperReportingV1alpha1SlowStartRateMetricSet setName(java.lang.String name) {
    this.name = name;
    return this;
  }

  @Override
  public GooglePlayDeveloperReportingV1alpha1SlowStartRateMetricSet set(String fieldName, Object value) {
    return (GooglePlayDeveloperReportingV1alpha1SlowStartRateMetricSet) super.set(fieldName, value);
  }

  @Override
  public GooglePlayDeveloperReportingV1alpha1SlowStartRateMetricSet clone() {
    return (GooglePlayDeveloperReportingV1alpha1SlowStartRateMetricSet) super.clone();
  }
