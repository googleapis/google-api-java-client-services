 ...
 * apps, see [Display cards and dialogs in columns](https://developers.google.com/workspace/chat
 * /format-structure-card-dialog#display_cards_and_dialogs_in_columns). The height of each column is
 * determined by the taller column. For example, if the first column is taller than the second
 * column, both columns have the height of the first column. Because each column can contain a
 * different number of widgets, you can't define rows or align widgets between the columns. Columns
 * are displayed side-by-side. You can customize the width of each column using the
