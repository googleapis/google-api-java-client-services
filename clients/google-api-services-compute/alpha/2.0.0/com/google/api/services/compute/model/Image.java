...
public final class Image extends com.google.api.client.json.GenericJson {
   ...
   * create. For example,centos-stream-9 is a publicly available image family. For more information,
   * see Image family best practices.
   *
   * When creating disks, you can specify an image family instead of a specific image name. The
   * image family always returns its latest image that is not deprecated. The name of the image
   ...
   * images. To see a list of available options, see theguestOSfeatures[].type parameter.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * be inherited by the disk created from the image.
   *
   * The disk with locked flag set to true will be prohibited from performing the operations below:
   * - R/W or R/O disk attach    - Disk detach, if disk is created via create-on-create    - Create
   * images    - Create snapshots    - Create disk clone (create disk from the current disk)
   *
   * The image with the locked field set to true will be prohibited from performing the operations
   * below:        - Create images from the current image    - Update the locked field for the
   * current image
   *
   * The instance with at least one disk with locked flag set to true will be prohibited from
   * performing the operations below:        - Secondary disk attach    - Create instant snapshot
   * - Create machine images    - Create instance template    - Delete the instance with --keep-disk
   ...
   * accessible.
   *
   * The rollout policy for this image is read-only, except for allowlisted users. This field might
   * not be configured. To view the latest non-deprecated image in a specific zone, use
   * theimageFamilyViews.get method.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  ...
  public static final class RawDisk extends com.google.api.client.json.GenericJson {
    ...
    /**
     * The format used to encode and transmit the block device, which should beTAR. This is just a
