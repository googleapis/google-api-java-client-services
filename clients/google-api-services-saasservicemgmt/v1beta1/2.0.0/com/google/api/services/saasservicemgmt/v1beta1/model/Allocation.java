/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.saasservicemgmt.v1beta1.model;

/**
 * Allocation defines a set of weighted flag variants, specifying how traffic is split based on the
 * randomization unit.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the App Lifecycle Manager API. For a detailed explanation
 * see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class Allocation extends com.google.api.client.json.GenericJson {

  /**
   * Optional. Description of the allocation. Max length: 500 bytes.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String description;

  /**
   * Required. Allocation ID. Max length: 128 bytes.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String id;

  /**
   * Required. Key of the context attribute that is used for traffic splitting.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String randomizedOn;

  /**
   * Required. Slots defines the weighted distribution of variants.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.util.List<AllocationSlot> slots;

  /**
   * Optional. Description of the allocation. Max length: 500 bytes.
   * @return value or {@code null} for none
   */
  public java.lang.String getDescription() {
    return description;
  }

  /**
   * Optional. Description of the allocation. Max length: 500 bytes.
   * @param description description or {@code null} for none
   */
  public Allocation setDescription(java.lang.String description) {
    this.description = description;
    return this;
  }

  /**
   * Required. Allocation ID. Max length: 128 bytes.
   * @return value or {@code null} for none
   */
  public java.lang.String getId() {
    return id;
  }

  /**
   * Required. Allocation ID. Max length: 128 bytes.
   * @param id id or {@code null} for none
   */
  public Allocation setId(java.lang.String id) {
    this.id = id;
    return this;
  }

  /**
   * Required. Key of the context attribute that is used for traffic splitting.
   * @return value or {@code null} for none
   */
  public java.lang.String getRandomizedOn() {
    return randomizedOn;
  }

  /**
   * Required. Key of the context attribute that is used for traffic splitting.
   * @param randomizedOn randomizedOn or {@code null} for none
   */
  public Allocation setRandomizedOn(java.lang.String randomizedOn) {
    this.randomizedOn = randomizedOn;
    return this;
  }

  /**
   * Required. Slots defines the weighted distribution of variants.
   * @return value or {@code null} for none
   */
  public java.util.List<AllocationSlot> getSlots() {
    return slots;
  }

  /**
   * Required. Slots defines the weighted distribution of variants.
   * @param slots slots or {@code null} for none
   */
  public Allocation setSlots(java.util.List<AllocationSlot> slots) {
    this.slots = slots;
    return this;
  }

  @Override
  public Allocation set(String fieldName, Object value) {
    return (Allocation) super.set(fieldName, value);
  }

  @Override
  public Allocation clone() {
    return (Allocation) super.clone();
  }

}
