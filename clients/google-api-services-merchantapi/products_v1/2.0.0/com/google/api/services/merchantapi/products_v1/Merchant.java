...
public class Merchant extends com.google.api.client.googleapis.services.json.AbstractGoogleJsonClient {
  ...
  public class Accounts {
    ...
    public class ProductInputs {
       ...
       *        be the unpadded base64url encoding of this string, which is `ZW5-VVN-c2t1LzEyMw`. The full
       *        resource name for the product would be `accounts/123/productInputs/ZW5-VVN-c2t1LzEyMw`. 2.
       ...
       *        the two formats.
         ...
         * of the product identifier (like `offer_id`) contains characters such as `/`, `%`, or `~`.
         * * Example: To represent the product ID `en~US~sku/123`, the `{productInput}` segment must
         * be the unpadded base64url encoding of this string, which is `ZW5-VVN-c2t1LzEyMw`. The
       ...
       represent the product ID `en~US~sku/123`, the `{productInput}` segment must be the unpadded
       base64url encoding of this string, which is `ZW5-VVN-c2t1LzEyMw`. The full resource name for the
       product would be `accounts/123/productInputs/ZW5-VVN-c2t1LzEyMw`. 2. **Plain Format**: The
       ...
       of tilde (`~`) characters in the `{productInput}` segment is used to differentiate between the two
       formats.
         */
       ...
       *        `en~US~sku/123`, the `{productinput}` segment must be the unpadded base64url encoding of
       *        this string, which is `ZW5-VVN-c2t1LzEyMw`. The full resource name for the product would
       *        be `accounts/123/productInputs/ZW5-VVN-c2t1LzEyMw`. 2. **Plain Format**: The
       *        `{productinput}` segment is the tilde-separated string
       *        `content_language~feed_label~offer_id`. This format is suitable only when
       *        `content_language`, `feed_label`, and `offer_id` do not contain URL-problematic characters
       *        like `/`, `%`, or `~`. We recommend using the **Encoded Format** for all product IDs to
