...
public final class DateOperatorOptions extends com.google.api.client.json.GenericJson {
   ...
   * property's name is *closeDate*, then queries like *closedafter:value* show results only where
   * the value of the property named *closeDate* is later than *value*. The operator name can only
   ...
   * like *closedon:value* show results only where the value of the *closeDate* property matches
   ...
   * the indexed datasource. The operator name can only contain lowercase letters (a-z). The maximum
