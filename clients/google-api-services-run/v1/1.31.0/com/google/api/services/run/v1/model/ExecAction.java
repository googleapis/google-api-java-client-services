...
/**
 * Cloud Run fully managed: not supported Cloud Run for Anthos: supported ExecAction describes a
...
public final class ExecAction extends com.google.api.client.json.GenericJson {
  ...
  /**
   * (Optional) Cloud Run fully managed: not supported Cloud Run for Anthos: supported Command is
   * the command line to execute inside the container, the working directory for the command is root
   * ('/') in the container's filesystem. The command is simply exec'd, it is not run inside a
