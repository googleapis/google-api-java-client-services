 ...
 * user's device, e.g., 26. * `versionCode` (int64): version of the app that was running on the
 * user's device. * `deviceModel` (string): unique identifier of the user's device model. The form
 * of the identifier is 'deviceBrand/device', where deviceBrand corresponds to Build.BRAND and
 * device corresponds to Build.DEVICE, e.g., google/coral. * `deviceBrand` (string): unique
 * identifier of the user's device brand, e.g., google. * `deviceType` (string): the type (also
 * known as form factor) of the user's device, e.g., PHONE. * `countryCode` (string): the country or
 * region of the user's device based on their IP address, represented as a 2-letter ISO-3166 code
 * (e.g. US for the United States). * `deviceRamBucket` (int64): RAM of the device, in MB, in
 * buckets (e.g., 1024 for 1-1.5GB, 4096 for 4-6GB). * `deviceSocMake` (string): Make of the
