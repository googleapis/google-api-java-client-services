...
public final class SecurityPolicyRuleRateLimitOptions extends com.google.api.client.json.GenericJson {
   ...
   * default value if    "enforceOnKey" is not configured.    - IP: The source IP address of    the
   * request is the key. Each IP has this limit enforced    separately.    - HTTP_HEADER: The value
   * of the HTTP    header whose name is configured under "enforceOnKeyName". The key    value is
   * truncated to the first 128 bytes of the header value. If no    such header is present in the
   ...
   * truncated to the first 128 bytes.     - SNI: Server name indication in the TLS session of the
   * HTTPS request. The key value is truncated to the first 128 bytes. The    key type defaults to
   * ALL on a HTTP session.     - REGION_CODE: The country/region from which the request
   ...
   * configured with the    security policy. If there is no "userIpRequestHeaders" configuration or
   * an IP address cannot be resolved from it, the key type defaults toIP.     - ASN: The autonomous
   * system number of the originating    client. If not available, the key type defaults toALL.    -
   ...
   *
   * For "fairshare" action, this value is limited to ALL i.e. a single rate limit threshold is
