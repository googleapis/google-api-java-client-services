...
public class Sheets extends com.google.api.client.googleapis.services.json.AbstractGoogleJsonClient {
  ...
  public class Spreadsheets {
     ...
     * mask listing your desired fields using the `fields` URL parameter in HTTP * Set the
     * includeGridData URL parameter to true. If a field mask is set, the `includeGridData` parameter is
     * ignored For large spreadsheets, as a best practice, retrieve only the specific spreadsheet fields
     * that you want. To retrieve only subsets of spreadsheet data, use the ranges URL parameter. Ranges
     * are specified using [A1 notation](/sheets/api/guides/concepts#cell). You can define a single cell
     ...
     * once (for example, `?ranges=A1:D5=Sheet2!A1:C4`). Limiting the range returns only the portions of
     * the spreadsheet that intersect the requested ranges.
     *
    ...
    public class Get extends SheetsRequest<com.google.api.services.sheets.v4.model.Spreadsheet> {
       ...
       * parameter is ignored For large spreadsheets, as a best practice, retrieve only the specific
       * spreadsheet fields that you want. To retrieve only subsets of spreadsheet data, use the ranges
       * URL parameter. Ranges are specified using [A1 notation](/sheets/api/guides/concepts#cell). You
       * can define a single cell (for example, `A1`) or multiple cells (for example, `A1:D5`). You can
       * also get cells from other sheets within the same spreadsheet (for example, `Sheet2!A1:C4`) or
       * retrieve multiple ranges at once (for example, `?ranges=A1:D5=Sheet2!A1:C4`). Limiting the
       * range returns only the portions of the spreadsheet that intersect the requested ranges.
       *
       * Create a request for the method "spreadsheets.get".
       *
       * This request holds the parameters needed by the the sheets server.  After setting any optional
     ...
     * of 2 ways: * Specify a field mask listing your desired fields using the `fields` URL parameter in
     * HTTP * Set the includeGridData parameter to true. If a field mask is set, the `includeGridData`
     * parameter is ignored For large spreadsheets, as a best practice, retrieve only the specific
    ...
    public class GetByDataFilter extends SheetsRequest<com.google.api.services.sheets.v4.model.Spreadsheet> {
       ...
       * include grid data one of 2 ways: * Specify a field mask listing your desired fields using the
       * `fields` URL parameter in HTTP * Set the includeGridData parameter to true. If a field mask is
       * set, the `includeGridData` parameter is ignored For large spreadsheets, as a best practice,
       * retrieve only the specific spreadsheet fields that you want.
       *
       * Create a request for the method "spreadsheets.getByDataFilter".
       *
       * This request holds the parameters needed by the the sheets server.  After setting any optional
