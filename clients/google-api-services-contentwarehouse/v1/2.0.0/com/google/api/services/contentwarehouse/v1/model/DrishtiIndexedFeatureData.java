/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.contentwarehouse.v1.model;

/**
 * Model definition for DrishtiIndexedFeatureData.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the Document AI Warehouse API. For a detailed explanation
 * see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class DrishtiIndexedFeatureData extends com.google.api.client.json.GenericJson {

  /**
   * If extra is present it must be of the same length as index and value.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.util.List<DrishtiFeatureExtra> extra;

  static {
    // hack to force ProGuard to consider DrishtiFeatureExtra used, since otherwise it would be stripped out
    // see https://github.com/google/google-api-java-client/issues/543
    com.google.api.client.util.Data.nullOf(DrishtiFeatureExtra.class);
  }

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private DrishtiFeatureExtra generalExtra;

  /**
   * Indexed data. index and value must be of the same length.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key @com.google.api.client.json.JsonString
  private java.util.List<java.math.BigInteger> index;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.util.List<java.lang.Float> value;

  /**
   * If extra is present it must be of the same length as index and value.
   * @return value or {@code null} for none
   */
  public java.util.List<DrishtiFeatureExtra> getExtra() {
    return extra;
  }

  /**
   * If extra is present it must be of the same length as index and value.
   * @param extra extra or {@code null} for none
   */
  public DrishtiIndexedFeatureData setExtra(java.util.List<DrishtiFeatureExtra> extra) {
    this.extra = extra;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public DrishtiFeatureExtra getGeneralExtra() {
    return generalExtra;
  }

  /**
   * @param generalExtra generalExtra or {@code null} for none
   */
  public DrishtiIndexedFeatureData setGeneralExtra(DrishtiFeatureExtra generalExtra) {
    this.generalExtra = generalExtra;
    return this;
  }

  /**
   * Indexed data. index and value must be of the same length.
   * @return value or {@code null} for none
   */
  public java.util.List<java.math.BigInteger> getIndex() {
    return index;
  }

  /**
   * Indexed data. index and value must be of the same length.
   * @param index index or {@code null} for none
   */
  public DrishtiIndexedFeatureData setIndex(java.util.List<java.math.BigInteger> index) {
    this.index = index;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public java.util.List<java.lang.Float> getValue() {
    return value;
  }

  /**
   * @param value value or {@code null} for none
   */
  public DrishtiIndexedFeatureData setValue(java.util.List<java.lang.Float> value) {
    this.value = value;
    return this;
  }

  @Override
  public DrishtiIndexedFeatureData set(String fieldName, Object value) {
    return (DrishtiIndexedFeatureData) super.set(fieldName, value);
  }

  @Override
  public DrishtiIndexedFeatureData clone() {
    return (DrishtiIndexedFeatureData) super.clone();
  }

}
