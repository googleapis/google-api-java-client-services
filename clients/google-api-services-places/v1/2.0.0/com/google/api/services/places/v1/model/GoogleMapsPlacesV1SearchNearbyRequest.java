...
public final class GoogleMapsPlacesV1SearchNearbyRequest extends com.google.api.client.json.GenericJson {
   ...
   * https://developers.google.com/maps/documentation/places/web-service/place-types. If there are
   * any conflicting primary types, i.e. a type appears in both included_primary_types and
   * excluded_primary_types, an INVALID_ARGUMENT error is returned. If a Place type is specified
   * with multiple type restrictions, only places that satisfy all of the restrictions are returned.
   * For example, if we have {included_types = ["restaurant"], excluded_primary_types =
   ...
   * https://developers.google.com/maps/documentation/places/web-service/place-types. If the client
   * provides both included_types (e.g. restaurant) and excluded_types (e.g. cafe), then the
   * response should include places that are restaurant but not cafe. The response includes places
   * that match at least one of the included_types and none of the excluded_types. If there are any
   * conflicting types, i.e. a type appears in both included_types and excluded_types, an
   * INVALID_ARGUMENT error is returned. If a Place type is specified with multiple type
   * restrictions, only places that satisfy all of the restrictions are returned. For example, if we
   * have {included_types = ["restaurant"], excluded_primary_types = ["restaurant"]}, the returned
   * places provide "restaurant" related services but do not operate primarily as "restaurants".
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * only have a single primary type from the supported types table associated with it. If there are
   * any conflicting primary types, i.e. a type appears in both included_primary_types and
   * excluded_primary_types, an INVALID_ARGUMENT error is returned. If a Place type is specified
   * with multiple type restrictions, only places that satisfy all of the restrictions are returned.
   * For example, if we have {included_types = ["restaurant"], excluded_primary_types =
   ...
   * https://developers.google.com/maps/documentation/places/web-service/place-types. If there are
   * any conflicting types, i.e. a type appears in both included_types and excluded_types, an
   * INVALID_ARGUMENT error is returned. If a Place type is specified with multiple type
   * restrictions, only places that satisfy all of the restrictions are returned. For example, if we
   * have {included_types = ["restaurant"], excluded_primary_types = ["restaurant"]}, the returned
   * places provide "restaurant" related services but do not operate primarily as "restaurants".
