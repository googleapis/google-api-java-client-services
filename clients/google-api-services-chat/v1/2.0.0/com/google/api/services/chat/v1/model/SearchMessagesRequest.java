...
public final class SearchMessagesRequest extends com.google.api.client.json.GenericJson {

  /**
   * Required. A search query. The query can specify one or more search keywords, which are used to
   * filter the results, You can also filter the results using the following message fields: -
   * `create_time`: Accepts a timestamp in [RFC-3339](https://www.rfc-editor.org/rfc/rfc3339) format
   * and the supported comparison operators are: `<` and `>=`. - `sender.name`: The resource name of
   * the sender (`users/{user}`). Only supports `=`. You can use the e-mail as an alias for
   * `{user}`. For example, `users/example@gmail.com`, where `example@gmail.com` is the e-mail of
   * the Google Chat user. - `space.name`: The resource name of the space where the message is
   * posted. (`spaces/{space}`). Only supports `=`. If this filter is not set, the search is
   * performed across all direct messages and spaces the user has access to as a space member. -
   ...
   * `https://www.googleapis.com/auth/chat.users.readstate` Across different fields, only `AND`
   * operators are supported. A valid example is `sender.name = "users/1234567890" AND is_unread()`.
   * The word `AND` is optional and is implied if omitted. For example, `sender.name =
   * "users/1234567890" is_unread()` is valid and is equivalent to the previous example. An invalid
   * example is `sender.name = "users/1234567890" OR is_unread()` because `OR` is not supported
   * between different fields. Among the same field: - `create_time` supports only `AND`, and can
   * only be used to represent an interval, such as `create_time >= "2022-01-01T00:00:00+00:00" AND
   * create_time < "2023-01-01T00:00:00+00:00"`. - `sender.name` supports only the `OR` operator,
   * for example: `sender.name = "users/1234567890" OR sender.name = "users/0987654321"`. -
   * `space.name` supports only the `OR` operator, for example: `space.name = "spaces/ABCDEFGH" OR
   * space.name = "spaces/QWERTYUI"`. - `space.display_name` supports the operators `AND` and `OR`,
   ...
   * user or both. Parentheses are required to disambiguate operator precedence when combining `AND`
   * and `OR` operators in the same query. For example: `(sender.name="users/me" OR
   * sender.name="users/123456") AND is_unread()`. Otherwise, parentheses are optional. The
   * following example queries are valid: ``` "Pending reports" AND create_time >=
   * "2023-01-01T00:00:00Z" sender.name = "users/example@gmail.com"
   * annotations.user_mentions.user.name:"users/0987654321" attachment:* AND space.name =
   * "spaces/ABCDEFGH" tasks AND is_unread() AND sender.name = "users/1234567890" "things to do"
   * "urgent" (sender.name = "users/1234567890") AND (create_time < "2023-05-01T00:00:00Z") tasks
   * AND space.name = "spaces/ABCDEFGH" AND has_link() "project one" is_unread()
   * space.display_name:Project tasks ``` The maximum query length is 1,000 characters. Invalid
   ...
   * Preview](https://developers.google.com/workspace/preview). The default ordering is `create_time
   * desc`. Only a single order per query (`create_time` or `relevance`) is supported. Only
   * descending order (`desc`) is supported, and it must be specified after the order attribute.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * value. If unspecified, at most 25 are returned. The maximum value is 100. If you use a value
   * more than 100, it's automatically changed to 100.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  ...
  /**
   * Optional. A token, received from the previous search messages call. Provide this parameter to
   ...
   */
  public SearchMessagesRequest setOrderBy(java.lang.String orderBy) {
   ...
   */
  public SearchMessagesRequest setPageSize(java.lang.Integer pageSize) {
   ...
   */
  public SearchMessagesRequest setPageToken(java.lang.String pageToken) {
   ...
   */
  public SearchMessagesRequest setView(java.lang.String view) {
    this.view = view;
    return this;
  }

  @Override
  public SearchMessagesRequest set(String fieldName, Object value) {
    return (SearchMessagesRequest) super.set(fieldName, value);
  }

  @Override
  public SearchMessagesRequest clone() {
    return (SearchMessagesRequest) super.clone();
  }
