...
public final class Network extends com.google.api.client.json.GenericJson {
  ...
  /**
   * Google Compute Engine network where the virtual machines are created. Specify the short name,
   ...
   * created in is a Legacy network, then the IP address is allocated from the IPv4Range. If the
   * network that the instance is being created in is an auto Subnet Mode Network, then only network
   * name should be specified (not the subnetwork_name) and the IP address is created from the
   * IPCidrRange of the subnetwork that exists in that zone for that network. If the network that
   * the instance is being created in is a custom Subnet Mode Network, then the subnetwork_name must
   * be specified and the IP address is created from the IPCidrRange of the subnetwork.If specified,
   * the subnetwork must exist in the same region as the App Engine flexible environment
   * application.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
