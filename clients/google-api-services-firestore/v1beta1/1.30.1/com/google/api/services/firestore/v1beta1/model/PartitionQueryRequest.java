...
public final class PartitionQueryRequest extends com.google.api.client.json.GenericJson {
   ...
   *
   * For example, if `partition_count` = 10 and `page_size` = 8, the first call to PartitionQuery
   * will return up to 8 partitions and a `next_page_token` if more results exist. A second call to
   ...
   * Thus, using multiple sets of results will require merging the different result sets.
   *
   * For example, two subsequent calls using a page_token may return:
   *
   *  * cursor B, cursor M, cursor Q  * cursor A, cursor U, cursor W
   *
   * To obtain a complete result set ordered with respect to the results of the query supplied to
   * PartitionQuery, the results sets should be merged: cursor A, cursor B, cursor M, cursor Q,
   * cursor U, cursor W
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * pages of results. The number must be strictly positive. The actual number of partitions
   * returned may be fewer.
   *
   * For example, this may be set to one fewer than the number of parallel queries to be run, or in
   * running a data pipeline job, one fewer than the number of workers or compute instances
   * available.
   * The value may be {@code null}.
   */
  ...
  /**
   * A structured query. Filters, order bys, limits, offsets, and start/end cursors are not
   * supported.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
