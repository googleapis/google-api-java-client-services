...
public final class HistogramQuery extends com.google.api.client.json.GenericJson {
   ...
   * list of buckets)`: Count the number of matching entity within each bucket. Data types: *
   ...
   * example, [1, 2, 3] and ["one", "two", "three"]. Built-in constants: * MIN (minimum number
   * similar to java Double.MIN_VALUE) * MAX (maximum number similar to java Double.MAX_VALUE)
   * Built-in functions: * bucket(start, end[, label]): bucket built-in function creates a bucket
   * with range of start, end). Note that the end is exclusive. For example, bucket(1, MAX,
   * "positive number") or bucket(1, 10). Job histogram facets: * company_id: histogram by
   ...
   * publish_time_in_month: histogram by the Job.publish_time in months. Must specify list of
   * numeric buckets in spec. * publish_time_in_year: histogram by the Job.publish_time in years.
   * Must specify list of numeric buckets in spec. * degree_type: histogram by the Job.degree_type.
   * For example, "Bachelors", "Masters". * job_level: histogram by the Job.job_level. For example,
