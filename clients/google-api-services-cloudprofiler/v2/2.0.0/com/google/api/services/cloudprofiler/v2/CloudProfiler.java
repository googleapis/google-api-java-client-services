...
public class CloudProfiler extends com.google.api.client.googleapis.services.json.AbstractGoogleJsonClient {
  ...
  public class Projects {
    ...
    public class Profiles {
       ...
       * collection._ The server ensures that the new profiles are created at a constant rate per
       * deployment, so the creation request may hang for some time until the next profile session is
       * available. The request may fail with ABORTED error if the creation is not available within ~1m,
       * the response will indicate the duration of the backoff the client should take before attempting
       * creating a profile again. The backoff duration is returned in google.rpc.RetryInfo extension on
       * the response status. To a gRPC client, the extension will be return as a binary-serialized proto
       * in the trailing metadata item named "google.rpc.retryinfo-bin".
       *
      ...
      public class Create extends CloudProfilerRequest<com.google.api.services.cloudprofiler.v2.model.Profile> {
         ...
         * profile collection._ The server ensures that the new profiles are created at a constant rate
         * per deployment, so the creation request may hang for some time until the next profile session
         * is available. The request may fail with ABORTED error if the creation is not available within
         * ~1m, the response will indicate the duration of the backoff the client should take before
         * attempting creating a profile again. The backoff duration is returned in google.rpc.RetryInfo
         * extension on the response status. To a gRPC client, the extension will be return as a binary-
         * serialized proto in the trailing metadata item named "google.rpc.retryinfo-bin".
         *
         * Create a request for the method "profiles.create".
         *
