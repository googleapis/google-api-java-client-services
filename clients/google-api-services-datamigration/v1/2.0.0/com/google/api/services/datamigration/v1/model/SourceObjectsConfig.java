/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.datamigration.v1.model;

/**
 * List of configurations for the source objects to be migrated.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the Database Migration API. For a detailed explanation
 * see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class SourceObjectsConfig extends com.google.api.client.json.GenericJson {

  /**
   * Optional. The list of the objects to be migrated.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.util.List<SourceObjectConfig> objectConfigs;

  static {
    // hack to force ProGuard to consider SourceObjectConfig used, since otherwise it would be stripped out
    // see https://github.com/google/google-api-java-client/issues/543
    com.google.api.client.util.Data.nullOf(SourceObjectConfig.class);
  }

  /**
   * Optional. The objects selection type of the migration job.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String objectsSelectionType;

  /**
   * Optional. The list of the objects to be migrated.
   * @return value or {@code null} for none
   */
  public java.util.List<SourceObjectConfig> getObjectConfigs() {
    return objectConfigs;
  }

  /**
   * Optional. The list of the objects to be migrated.
   * @param objectConfigs objectConfigs or {@code null} for none
   */
  public SourceObjectsConfig setObjectConfigs(java.util.List<SourceObjectConfig> objectConfigs) {
    this.objectConfigs = objectConfigs;
    return this;
  }

  /**
   * Optional. The objects selection type of the migration job.
   * @return value or {@code null} for none
   */
  public java.lang.String getObjectsSelectionType() {
    return objectsSelectionType;
  }

  /**
   * Optional. The objects selection type of the migration job.
   * @param objectsSelectionType objectsSelectionType or {@code null} for none
   */
  public SourceObjectsConfig setObjectsSelectionType(java.lang.String objectsSelectionType) {
    this.objectsSelectionType = objectsSelectionType;
    return this;
  }

  @Override
  public SourceObjectsConfig set(String fieldName, Object value) {
    return (SourceObjectsConfig) super.set(fieldName, value);
  }

  @Override
  public SourceObjectsConfig clone() {
    return (SourceObjectsConfig) super.clone();
  }

}
