...
public class DeveloperKnowledge extends com.google.api.client.googleapis.services.json.AbstractGoogleJsonClient {
  ...
  public class Documents {
    ...
    public class SearchDocumentChunks extends DeveloperKnowledgeRequest<com.google.api.services.developerknowledge.v1.model.SearchDocumentChunksResponse> {
       ...
       * `"2025-01-01T00:00:00Z"`. Note: Field names must be in `snake_case` (e.g., `data_source`).
       * Values on the right-hand side of filtering expressions must be string literals enclosed in
       * double quotes (e.g., `"docs.cloud.google.com"`). You can combine expressions using `AND`,
       * `OR`, and `NOT` (or `-`) logical operators. `OR` has higher precedence than `AND`. Use
       * parentheses for explicit precedence grouping. Examples: * `data_source =
     ...
     Timestamps must be in RFC-3339 format, e.g., `"2025-01-01T00:00:00Z"`. Note: Field names must be in
     `snake_case` (e.g., `data_source`). Values on the right-hand side of filtering expressions must be
     string literals enclosed in double quotes (e.g., `"docs.cloud.google.com"`). You can combine
     expressions using `AND`, `OR`, and `NOT` (or `-`) logical operators. `OR` has higher precedence
     than `AND`. Use parentheses for explicit precedence grouping. Examples: * `data_source =
     "docs.cloud.google.com" OR data_source = "firebase.google.com"` * `data_source !=
     "firebase.google.com"` * `update_time < "2024-01-01T00:00:00Z"` * `update_time >=
