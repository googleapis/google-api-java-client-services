 ...
 * identify particular VM instances. Different services can support different monitored resource
 * types. The following are specific rules to service defined monitored resources for Monitoring and
 * Logging: * The `type`, `display_name`, `description`, `labels` and `launch_stage` fields are all
 ...
 * allowed per service. * Maximum of default 10 labels per monitored resource is allowed. The
...
public final class MonitoredResourceDescriptor extends com.google.api.client.json.GenericJson {
   ...
   * key name must follow: * Only upper and lower-case letters, digits and underscores (_) are
   * allowed. * Label name must start with a letter or digit. * The maximum length of a label name
   * is 100 characters. For example, an individual Google Cloud SQL database is identified by values
   * for the labels `database_id` and `location`.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * resource name must follow: * Only upper and lower-case letters and digits are allowed. * It
   * must start with upper case character and is recommended to use Upper Camel Case style. * The
   * maximum number of characters allowed for the relative_resource_name is 100. Note there are
