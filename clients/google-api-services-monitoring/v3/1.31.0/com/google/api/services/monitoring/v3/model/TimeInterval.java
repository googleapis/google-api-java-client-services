 ...
 * the metric value. The end time must not be earlier than the start time. When writing data points,
 * the start time must not be more than 25 hours in the past and the end time must not be more than
 * five minutes in the future. For GAUGE metrics, the startTime value is technically optional; if no
 * value is specified, the start time defaults to the value of the end time, and the interval
 * represents a single point in time. If both start and end times are specified, they must be
 * identical. Such an interval is valid only for GAUGE metrics, which are point-in-time
 * measurements. The end time of a new interval must be at least a millisecond after the end time of
 ...
 * interval. The start time of a new interval must be at least a millisecond after the end time of
 * the previous interval because intervals are closed. If the start time of a new interval is the
