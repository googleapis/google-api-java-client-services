 ...
 * can't be represented as JSON, such as raw binary or an HTML page.This message can be used both in
 * streaming and non-streaming API methods in the request as well as the response.It can be used as
 * a top-level request field, which is convenient if one wants to extract parameters from either the
 * URL or HTTP template into the request fields and also want access to the raw HTTP body.Example:
 * message GetResourceRequest { // A unique request id. string request_id = 1; // The raw HTTP body
 * is bound to this field. google.api.HttpBody http_body = 2; } service ResourceService { rpc
