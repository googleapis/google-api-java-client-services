...
/**
 * App Engine HTTP target. The task will be delivered to the App Engine application hostname
 * specified by its AppEngineHttpTarget and AppEngineHttpRequest. The documentation for
 * AppEngineHttpRequest explains how the task's host URL is constructed. Using AppEngineHttpTarget
 * requires [`appengine.applications.get`](https://cloud.google.com/appengine/docs/admin-api/access-
...
public final class AppEngineHttpTarget extends com.google.api.client.json.GenericJson {
  ...
  /**
   * Overrides for the task-level app_engine_routing. If set, `app_engine_routing_override` is used
   * for all tasks in the queue, no matter what the setting is for the task-level
   * app_engine_routing.
