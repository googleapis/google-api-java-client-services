...
public final class GooglePrivacyDlpV2StoredInfoTypeVersion extends com.google.api.client.json.GenericJson {
   ...
   *
   * For example, some of the data for stored custom dictionaries is put in the user's Google Cloud
   * Storage bucket, and if this data is modified or deleted by the user or another system, the
   * dictionary becomes invalid.
   *
   * If any errors occur, fix the problem indicated by the error message and use the
   * UpdateStoredInfoType API method to create another version of the storedInfoType to continue
   * using it, reusing the same `config` if it was not the source of the error.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
