 ...
 * # Transactions
 *
 * Each session can have at most one active transaction at a time (note that standalone reads and
 * queries use a transaction internally and do count towards the one transaction limit). After the
 * active transaction is completed, the session can immediately be re-used for the next transaction.
 ...
 * Spanner. These transactions rely on      pessimistic locking and, if necessary, two-phase commit.
 * Locking read-write transactions may abort, requiring the      application to retry.
 *
 *   2. Snapshot read-only. This transaction type provides guaranteed      consistency across
 * several reads, but does not allow      writes. Snapshot read-only transactions can be configured
 * to      read at timestamps in the past. Snapshot read-only      transactions do not need to be
 * committed.
 *
 ...
 *
 * For transactions that only read, snapshot read-only transactions provide simpler semantics and
 * are almost always faster. In particular, read-only transactions do not take locks, so they do not
 ...
 *
 * Transactions may only read/write data in a single database. They may, however, read/write data in
 * different tables within that database.
 *
 ...
 * not been terminated by Commit or Rollback.  Long periods of inactivity at the client may cause
 * Cloud Spanner to release a transaction's locks and abort it.
 *
 * Conceptually, a read-write transaction consists of zero or more reads or SQL statements followed
 * by Commit. At any time before Commit, the client can send a Rollback request to abort the
 * transaction.
 *
 * ### Semantics
 *
 * Cloud Spanner can commit the transaction if all read locks it acquired are still valid at commit
 * time, and it is able to acquire write locks for all writes. Cloud Spanner can abort the
 * transaction for any reason. If a commit attempt returns `ABORTED`, Cloud Spanner guarantees that
 ...
 *
 * Snapshot read-only transactions provides a simpler method than locking read-write transactions
 * for doing several consistent reads. However, this type of transaction does not support writes.
 *
 * Snapshot transactions do not take locks. Instead, they work by choosing a Cloud Spanner
 * timestamp, then executing all reads at that timestamp. Since they do not acquire locks, they do
 * not block concurrent read-write transactions.
 *
 ...
 * policy is generous enough that most applications do not need to worry about this in practice.
 *
 * Snapshot read-only transactions do not need to call Commit or Rollback (and in fact are not
 * permitted to do so).
 *
 * To execute a snapshot transaction, the client specifies a timestamp bound, which tells Cloud
 ...
 * transactions can execute more quickly than strong or read-write transaction, because they are
 ...
 * modifications done by all transactions with a commit timestamp <= the read timestamp, and observe
 * none of the modifications done by transactions with a larger commit timestamp. They will block
 * until all conflicting transactions that may be assigned commit timestamps <= the read timestamp
 * have finished.
 *
 ...
 *
 * These modes do not require a "negotiation phase" to pick a timestamp. As a result, they execute
 * slightly faster than the equivalent boundedly stale concurrency modes. On the other hand,
 ...
 * ### Bounded Staleness
 *
 * Bounded staleness modes allow Cloud Spanner to pick the read timestamp, subject to a user-
 * provided staleness bound. Cloud Spanner chooses the newest timestamp within the staleness bound
 ...
 * timestamp.
 *
 * As a result of the two phase execution, bounded staleness reads are usually a little slower than
 * comparable exact staleness reads. However, they are typically able to return fresher results, and
 * are more likely to execute at the closest replica.
 *
 * Because the timestamp negotiation requires up-front knowledge of which rows will be read, it can
 * only be used with single-use read-only transactions.
 *
 ...
 * versions after they are one hour old. Because of this, Cloud Spanner cannot perform reads at read
 * timestamps more than one hour in the past. This restriction also applies to in-progress reads
 * and/or SQL queries whose timestamp become too old while executing. Reads and SQL queries with
 ...
 *
 * To reduce lock contention, this execution strategy only acquires read locks on rows that match
 * the WHERE clause of the statement. Additionally, the smaller per-partition transactions hold
 * locks for less time.
 *
 ...
 * expressible as the union of many statements which each access only    a single row of the table.
 *
 ...
 * The statement will be applied at least once to each    partition. It is strongly recommended that
 ...
 * instance, a UNIQUE INDEX violation, division by zero, or a    value that cannot be stored due to
 * schema constraints), then the    operation is stopped at that point and an error is returned. It
 * is    possible that at this point, some partitions have been committed (or even    committed
