...
public final class GoogleCloudApigeeV1SyncAuthorization extends com.google.api.client.json.GenericJson {
   ...
   *
   * For example, when you call  [getSyncAuthorization](getSyncAuthorization) an ETag is returned in
   * the response. Pass that ETag when calling the [setSyncAuthorization](setSyncAuthorization) to
   ...
   * using the following format: `serviceAccount:`service-account-name.
   *
   * The service-account-name is formatted like an email address. For example: `my-synchronizer-
   * manager-service_account@my_project_id.iam.gserviceaccount.com`
   *
   * You might specify multiple service accounts, for example, if you have multiple environments and
   * wish to assign a unique service account to each one.
   *
   * The service accounts must have **Apigee Synchronizer Manager** role. See also [Add service
   * accounts](/hybrid/precog-serviceaccounts).
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
