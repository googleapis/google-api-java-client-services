/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.cloudbuild.v1.model;

/**
 * OCI image to upload to Artifact Registry upon successful completion of all build steps.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the Cloud Build API. For a detailed explanation see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class Oci extends com.google.api.client.json.GenericJson {

  /**
   * Required. Path on the local file system where to find the container to upload. e.g.
   * /workspace/my-image.tar
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String file;

  /**
   * Required. Registry path to upload the container to. e.g. us-east1-docker.pkg.dev/my-project/my-
   * repo/my-image
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String registryPath;

  /**
   * Optional. Tags to apply to the uploaded image. e.g. latest, 1.0.0
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.util.List<java.lang.String> tags;

  /**
   * Required. Path on the local file system where to find the container to upload. e.g.
   * /workspace/my-image.tar
   * @return value or {@code null} for none
   */
  public java.lang.String getFile() {
    return file;
  }

  /**
   * Required. Path on the local file system where to find the container to upload. e.g.
   * /workspace/my-image.tar
   * @param file file or {@code null} for none
   */
  public Oci setFile(java.lang.String file) {
    this.file = file;
    return this;
  }

  /**
   * Required. Registry path to upload the container to. e.g. us-east1-docker.pkg.dev/my-project/my-
   * repo/my-image
   * @return value or {@code null} for none
   */
  public java.lang.String getRegistryPath() {
    return registryPath;
  }

  /**
   * Required. Registry path to upload the container to. e.g. us-east1-docker.pkg.dev/my-project/my-
   * repo/my-image
   * @param registryPath registryPath or {@code null} for none
   */
  public Oci setRegistryPath(java.lang.String registryPath) {
    this.registryPath = registryPath;
    return this;
  }

  /**
   * Optional. Tags to apply to the uploaded image. e.g. latest, 1.0.0
   * @return value or {@code null} for none
   */
  public java.util.List<java.lang.String> getTags() {
    return tags;
  }

  /**
   * Optional. Tags to apply to the uploaded image. e.g. latest, 1.0.0
   * @param tags tags or {@code null} for none
   */
  public Oci setTags(java.util.List<java.lang.String> tags) {
    this.tags = tags;
    return this;
  }

  @Override
  public Oci set(String fieldName, Object value) {
    return (Oci) super.set(fieldName, value);
  }

  @Override
  public Oci clone() {
    return (Oci) super.clone();
  }

}
