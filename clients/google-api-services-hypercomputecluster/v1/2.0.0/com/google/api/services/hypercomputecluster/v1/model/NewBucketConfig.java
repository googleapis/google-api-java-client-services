/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.hypercomputecluster.v1.model;

/**
 * When set in a StorageResourceConfig, indicates that a new [Google Cloud
 * Storage](https://cloud.google.com/storage) bucket should be created.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the Cluster Director API. For a detailed explanation see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class NewBucketConfig extends com.google.api.client.json.GenericJson {

  /**
   * Optional. Immutable. If set, indicates that the bucket should use
   * [Autoclass](https://cloud.google.com/storage/docs/autoclass).
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private GcsAutoclassConfig autoclass;

  /**
   * Required. Immutable. Name of the Cloud Storage bucket to create.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String bucket;

  /**
   * Optional. Immutable. If set, indicates that the bucket should use [hierarchical
   * namespaces](https://cloud.google.com/storage/docs/hns-overview).
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private GcsHierarchicalNamespaceConfig hierarchicalNamespace;

  /**
   * Optional. Immutable. If set, uses the provided storage class as the bucket's default storage
   * class.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String storageClass;

  /**
   * Optional. Immutable. If set, indicates that the bucket should use
   * [Autoclass](https://cloud.google.com/storage/docs/autoclass).
   * @return value or {@code null} for none
   */
  public GcsAutoclassConfig getAutoclass() {
    return autoclass;
  }

  /**
   * Optional. Immutable. If set, indicates that the bucket should use
   * [Autoclass](https://cloud.google.com/storage/docs/autoclass).
   * @param autoclass autoclass or {@code null} for none
   */
  public NewBucketConfig setAutoclass(GcsAutoclassConfig autoclass) {
    this.autoclass = autoclass;
    return this;
  }

  /**
   * Required. Immutable. Name of the Cloud Storage bucket to create.
   * @return value or {@code null} for none
   */
  public java.lang.String getBucket() {
    return bucket;
  }

  /**
   * Required. Immutable. Name of the Cloud Storage bucket to create.
   * @param bucket bucket or {@code null} for none
   */
  public NewBucketConfig setBucket(java.lang.String bucket) {
    this.bucket = bucket;
    return this;
  }

  /**
   * Optional. Immutable. If set, indicates that the bucket should use [hierarchical
   * namespaces](https://cloud.google.com/storage/docs/hns-overview).
   * @return value or {@code null} for none
   */
  public GcsHierarchicalNamespaceConfig getHierarchicalNamespace() {
    return hierarchicalNamespace;
  }

  /**
   * Optional. Immutable. If set, indicates that the bucket should use [hierarchical
   * namespaces](https://cloud.google.com/storage/docs/hns-overview).
   * @param hierarchicalNamespace hierarchicalNamespace or {@code null} for none
   */
  public NewBucketConfig setHierarchicalNamespace(GcsHierarchicalNamespaceConfig hierarchicalNamespace) {
    this.hierarchicalNamespace = hierarchicalNamespace;
    return this;
  }

  /**
   * Optional. Immutable. If set, uses the provided storage class as the bucket's default storage
   * class.
   * @return value or {@code null} for none
   */
  public java.lang.String getStorageClass() {
    return storageClass;
  }

  /**
   * Optional. Immutable. If set, uses the provided storage class as the bucket's default storage
   * class.
   * @param storageClass storageClass or {@code null} for none
   */
  public NewBucketConfig setStorageClass(java.lang.String storageClass) {
    this.storageClass = storageClass;
    return this;
  }

  @Override
  public NewBucketConfig set(String fieldName, Object value) {
    return (NewBucketConfig) super.set(fieldName, value);
  }

  @Override
  public NewBucketConfig clone() {
    return (NewBucketConfig) super.clone();
  }

}
