...
public final class Entitlement extends com.google.api.client.json.GenericJson {
  ...
  /**
   * Output only. The end time of the new offer, determined from the offer's specified end date. If
   * the offer des not have a specified end date then this field is not set. This field is populated
   * even if the entitlement isn't active yet. If there's no upcoming offer, the field is empty. *
   * If the entitlement is in the state ENTITLEMENT_ACTIVATION_REQUESTED, ENTITLEMENT_ACTIVE, or
   * ENTITLEMENT_PENDING_CANCELLATION, then this field is empty. * If the entitlement is in the
   * state ENTITLEMENT_PENDING_PLAN_CHANGE_APPROVAL or ENTITLEMENT_PENDING_PLAN_CHANGE, and the
   * upcoming offer has a specified end date, then this field is populated with the expected end
   * time of the upcoming offer, in the future. Otherwise, this field is empty. * If the entitlement
   ...
   * entitlement is in the state ENTITLEMENT_ACTIVATION_REQUESTED, this field isn't populated when
   * the entitlement isn't yet approved. After the entitlement is approved, this field is populated
   * with the effective time of the upcoming offer. * If the entitlement is in the state
   * ENTITLEMENT_ACTIVE or ENTITLEMENT_PENDING_CANCELLATION, this field isn't populated. * If the
   * entitlement is in the state ENTITLEMENT_PENDING_PLAN_CHANGE_APPROVAL, this field isn't
   * populated, because the entitlement change is waiting on approval. * If the entitlement is in
   * the state ENTITLEMENT_PENDING_PLAN_CHANGE, this field is populated with the expected effective
   * time of the upcoming offer, which is in the future. * If the entitlement is in the state
  ...
  /**
   * Output only. Upon a pending plan change, the name of the offer that the entitlement is
   * switching to. Only exists if the pending plan change is moving to an offer. This field isn't
   * populated for entitlements which aren't active yet. Format:
   ...
   * {service} part. * If the entitlement is in the state ENTITLEMENT_ACTIVATION_REQUESTED,
   * ENTITLEMENT_ACTIVE or ENTITLEMENT_PENDING_CANCELLATION, then this field is empty. * If the
   * entitlement is in the state ENTITLEMENT_PENDING_PLAN_CHANGE_APPROVAL or
   * ENTITLEMENT_PENDING_PLAN_CHANGE, then this field is populated with the upcoming offer. * If the
   * entitlement is in the state ENTITLEMENT_CANCELLED, then this is empty.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  ...
  /**
   * Output only. The duration of the new offer, in ISO 8601 duration format. This field is
   * populated for pending offer changes. It isn't populated for entitlements which aren't active
   * yet. If the offer has a specified end date instead of a duration, this field is empty. * If the
   * entitlement is in the state ENTITLEMENT_ACTIVATION_REQUESTED, ENTITLEMENT_ACTIVE, or
   * ENTITLEMENT_PENDING_CANCELLATION, this field is empty. * If the entitlement is in the state
   * ENTITLEMENT_PENDING_PLAN_CHANGE_APPROVAL or ENTITLEMENT_PENDING_PLAN_CHANGE, and the upcoming
   * offer doesn't have a specified end date, then this field is populated with the duration of the
   * upcoming offer. Otherwise, this field is empty. * If the entitlement is in the state
   * ENTITLEMENT_CANCELLED, then this field is empty.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  ...
  /**
   * Output only. The name of the offer that was procured. Field is empty if order wasn't made using
   * an offer. Format: 'projects/{project}/services/{service}/privateOffers/{offer}' OR
   * 'projects/{project}/services/{service}/standardOffers/{offer}', depending on whether the offer
   * is private or public. The {service} in the name is the listing service of the offer. It could
   * be either the product service that the offer is referencing, or a generic private offer parent
   * service. We recommend that you don't build your integration to rely on the meaning of this
   * {service} part. * If the entitlement is in the state ENTITLEMENT_ACTIVATION_REQUESTED, this
   * field is populated with the upcoming offer. * If the entitlement is in the state
   * ENTITLEMENT_ACTIVE, ENTITLEMENT_PENDING_CANCELLATION, ENTITLEMENT_PENDING_PLAN_CHANGE, or
   * ENTITLEMENT_PENDING_PLAN_CHANGE_APPROVAL, this field is populated with the current offer. * If
   * the entitlement is in the state ENTITLEMENT_CANCELLED, then this field is populated with the
   * latest offer that the order was associated with.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  ...
  /**
   * Output only. The offer duration of the current offer, in ISO 8601 duration format. This is
   * empty if the entitlement wasn't made using an offer, or if the offer has a specified end date
   * instead of a duration. * If the entitlement is in the state ENTITLEMENT_ACTIVATION_REQUESTED,
   * and the upcoming offer doesn't have a specified end date, then this field is populated with the
   * duration of the upcoming offer. Otherwise, this field is empty. * If the entitlement is in the
   * state ENTITLEMENT_ACTIVE, ENTITLEMENT_PENDING_CANCELLATION, ENTITLEMENT_PENDING_PLAN_CHANGE, or
   * ENTITLEMENT_PENDING_PLAN_CHANGE_APPROVAL, and the current offer doesn't have a specified end
   * date, then this field contains the duration of the current offer. Otherwise, this field is
   * empty. * If the entitlement is in the state ENTITLEMENT_CANCELLED, and the offer doesn't have a
   * specified end date, then this field is populated with the duration of the latest offer that the
   * order was associated with. Otherwise, this field is empty.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  ...
  /**
   * Output only. End time for the current term of the Offer associated with this entitlement. The
   * value of this field can change naturally over time due to auto-renewal, even if the offer isn't
   * changed. * If the entitlement is in the state ENTITLEMENT_ACTIVATION_REQUESTED, then: * If the
   * entitlement isn't approved yet approved, and the offer has a specified end date, then this
   * field is populated with the expected end time of the upcoming offer, in the future. Otherwise,
   * this field is empty. * If the entitlement is approved, then this field is populated with the
   * expected end time of the upcoming offer, in the future. This means that this field and the
   * field offer_duration can both exist. * If the entitlement is in the state ENTITLEMENT_ACTIVE or
   * ENTITLEMENT_PENDING_CANCELLATION, then this field is populated with the expected end time of
   * the current offer, in the future. This field's value is set regardless of whether the offer has
   * a specific end date or a duration. This means that this field and the field offer_duration can
   * both exist. * If the entitlement is in the state ENTITLEMENT_PENDING_PLAN_CHANGE_APPROVAL or
   * ENTITLEMENT_PENDING_PLAN_CHANGE: * If the entitlement's pricing model is usage based and the
   * associated offer is a private offer whose term has ended, then this field reflects the ACTUAL
   * end time of the entitlement's associated offer (in the past), even though the entitlement
   * associated with this private offer does not terminate at the end of that private offer's term.
   * * Otherwise, this is the expected end date of the current offer, in the future. * If the
   * entitlement is in the state ENTITLEMENT_CANCELLED, then this field is populated with the end
   * time, in the past, of the latest offer that the order was associated with. If the entitlement
   * was cancelled before any offer started, then this field is empty.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
