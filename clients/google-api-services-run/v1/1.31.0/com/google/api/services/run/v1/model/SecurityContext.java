...
/**
 * Cloud Run fully managed: not supported Cloud Run for Anthos: supported SecurityContext holds
 * security configuration that will be applied to a container. Some fields are present in both
 * SecurityContext and PodSecurityContext. When both are set, the values in SecurityContext take
 * precedence.
 *
...
public final class SecurityContext extends com.google.api.client.json.GenericJson {
  ...
  /**
   * (Optional) Cloud Run fully managed: not supported Cloud Run for Anthos: supported The UID to
   * run the entrypoint of the container process. Defaults to user specified in image metadata if
