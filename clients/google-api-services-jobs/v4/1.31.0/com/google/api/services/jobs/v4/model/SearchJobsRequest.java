...
public final class SearchJobsRequest extends com.google.api.client.json.GenericJson {
   ...
   * Job.employment_types, for example, "FULL_TIME", "PART_TIME". * company_size: histogram by
   * CompanySize, for example, "SMALL", "MEDIUM", "BIG". * publish_time_in_day: histogram by the
   * Job.posting_publish_time in days. Must specify list of numeric buckets in spec. *
   * publish_time_in_month: histogram by the Job.posting_publish_time in months. Must specify list
   * of numeric buckets in spec. * publish_time_in_year: histogram by the Job.posting_publish_time
   * in years. Must specify list of numeric buckets in spec. * degree_types: histogram by the
   * Job.degree_types, for example, "Bachelors", "Masters". * job_level: histogram by the
   * Job.job_level, for example, "Entry Level". * country: histogram by the country code of jobs,
   * for example, "US", "FR". * admin1: histogram by the admin1 code of jobs, which is a global
   * placeholder referring to the state, province, or the particular term a country uses to define
   * the geographic structure below the country level, for example, "CA", "IL". * city: histogram by
   * a combination of the "city name, admin1 code". For example, "Mountain View, CA", "New York,
   * NY". * admin1_country: histogram by a combination of the "admin1 code, country", for example,
   * "CA, US", "IL, US". * city_coordinate: histogram by the city center's GPS coordinates (latitude
   * and longitude), for example, 37.4038522,-122.0987765. Since the coordinates of a city center
   * can change, customers may need to refresh them periodically. * locale: histogram by the
