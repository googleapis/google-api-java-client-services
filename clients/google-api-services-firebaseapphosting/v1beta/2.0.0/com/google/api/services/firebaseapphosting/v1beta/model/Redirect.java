/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.firebaseapphosting.v1beta.model;

/**
 * Specifies redirect behavior for a domain.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the Firebase App Hosting API. For a detailed explanation
 * see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class Redirect extends com.google.api.client.json.GenericJson {

  /**
   * Optional. The status code to use in a redirect response. Must be a valid HTTP 3XX status code.
   * Defaults to 302 if not present.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key @com.google.api.client.json.JsonString
  private java.lang.Long status;

  /**
   * Required. The URI of the redirect's intended destination. This URI will be prepended to the
   * original request path. URI without a scheme are assumed to be HTTPS.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String uri;

  /**
   * Optional. The status code to use in a redirect response. Must be a valid HTTP 3XX status code.
   * Defaults to 302 if not present.
   * @return value or {@code null} for none
   */
  public java.lang.Long getStatus() {
    return status;
  }

  /**
   * Optional. The status code to use in a redirect response. Must be a valid HTTP 3XX status code.
   * Defaults to 302 if not present.
   * @param status status or {@code null} for none
   */
  public Redirect setStatus(java.lang.Long status) {
    this.status = status;
    return this;
  }

  /**
   * Required. The URI of the redirect's intended destination. This URI will be prepended to the
   * original request path. URI without a scheme are assumed to be HTTPS.
   * @return value or {@code null} for none
   */
  public java.lang.String getUri() {
    return uri;
  }

  /**
   * Required. The URI of the redirect's intended destination. This URI will be prepended to the
   * original request path. URI without a scheme are assumed to be HTTPS.
   * @param uri uri or {@code null} for none
   */
  public Redirect setUri(java.lang.String uri) {
    this.uri = uri;
    return this;
  }

  @Override
  public Redirect set(String fieldName, Object value) {
    return (Redirect) super.set(fieldName, value);
  }

  @Override
  public Redirect clone() {
    return (Redirect) super.clone();
  }

}
