...
public final class GoogleCloudDiscoveryengineV1betaTrainCustomModelRequestGcsTrainingInput extends com.google.api.client.json.GenericJson {
   ...
   * `gs:`. A newline delimited jsonl/ndjson file. For search-tuning model, each line should have
   * the _id, title and text. Example: `{"_id": "doc1", title: "relevant doc", "text": "relevant
   * text"}`
  ...
  /**
   * Cloud Storage training data path whose format should be `gs:`. The file should be in tsv
   * format. Each line should have the doc_id and query_id and score (number). For search-tuning
   * model, it should have the query-id corpus-id score as tsv file header. The score should be a
   * number in `[0, inf+)`. The larger the number is, the more relevant the pair is. Example: *
