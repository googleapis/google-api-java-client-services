...
public final class Reservation extends com.google.api.client.json.GenericJson {
   ...
   * slots during runtime if ignore_idle_slots is set to false. If edition is EDITION_UNSPECIFIED
   * and total slot_capacity of the reservation and its siblings exceeds the total slot_count of all
   * capacity commitments, the request will fail with `google.rpc.Code.RESOURCE_EXHAUSTED`. If
   * edition is any value but EDITION_UNSPECIFIED, then the above requirement is not needed. The
   ...
   * can decrease your baseline slots every few minutes. NOTE: for reservations in US or EU multi-
   * regions, slot capacity constraints are checked separately for default and auxiliary regions.
