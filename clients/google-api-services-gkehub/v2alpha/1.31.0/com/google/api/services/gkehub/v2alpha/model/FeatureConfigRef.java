/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.gkehub.v2alpha.model;

/**
 * Information of the FeatureConfig applied on the MembershipFeature.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the GKE Hub API. For a detailed explanation see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class FeatureConfigRef extends com.google.api.client.json.GenericJson {

  /**
   * Input only. Resource name of FeatureConfig, in the format:
   * `projects/{project}/locations/global/featureConfigs/{feature_config}`.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String config;

  /**
   * Output only. When the FeatureConfig was last applied and copied to FeatureSpec.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private String configUpdateTime;

  /**
   * Output only. An id that uniquely identify a FeatureConfig object.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String uuid;

  /**
   * Input only. Resource name of FeatureConfig, in the format:
   * `projects/{project}/locations/global/featureConfigs/{feature_config}`.
   * @return value or {@code null} for none
   */
  public java.lang.String getConfig() {
    return config;
  }

  /**
   * Input only. Resource name of FeatureConfig, in the format:
   * `projects/{project}/locations/global/featureConfigs/{feature_config}`.
   * @param config config or {@code null} for none
   */
  public FeatureConfigRef setConfig(java.lang.String config) {
    this.config = config;
    return this;
  }

  /**
   * Output only. When the FeatureConfig was last applied and copied to FeatureSpec.
   * @return value or {@code null} for none
   */
  public String getConfigUpdateTime() {
    return configUpdateTime;
  }

  /**
   * Output only. When the FeatureConfig was last applied and copied to FeatureSpec.
   * @param configUpdateTime configUpdateTime or {@code null} for none
   */
  public FeatureConfigRef setConfigUpdateTime(String configUpdateTime) {
    this.configUpdateTime = configUpdateTime;
    return this;
  }

  /**
   * Output only. An id that uniquely identify a FeatureConfig object.
   * @return value or {@code null} for none
   */
  public java.lang.String getUuid() {
    return uuid;
  }

  /**
   * Output only. An id that uniquely identify a FeatureConfig object.
   * @param uuid uuid or {@code null} for none
   */
  public FeatureConfigRef setUuid(java.lang.String uuid) {
    this.uuid = uuid;
    return this;
  }

  @Override
  public FeatureConfigRef set(String fieldName, Object value) {
    return (FeatureConfigRef) super.set(fieldName, value);
  }

  @Override
  public FeatureConfigRef clone() {
    return (FeatureConfigRef) super.clone();
  }

}
