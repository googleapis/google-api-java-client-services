...
public final class NodeConfig extends com.google.api.client.json.GenericJson {
  ...
  /**
   * The Kubernetes labels (key/value pairs) to apply to each node. The values in this field are
   * added to the set of default labels Kubernetes applies to nodes. This field has the following
   * restrictions: * Labels must use a valid Kubernetes syntax and character set, as defined in
   * https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#syntax-and-character-
   * set. * This field supports up to 1,024 total characters in a single request. Depending on the
   * Kubernetes version, keys in this field might conflict with the keys of the default labels,
   * which might change which of your labels are applied to the nodes. Assume that the behavior is
   * unpredictable and avoid label key conflicts. For more information about the default labels,
   ...
   * communicating with **gcr.io** (the [Artifact Registry](https://cloud.google.com/artifact-
   * registry/)). If unspecified, no scopes are added, unless Cloud Logging or Cloud Monitoring are
