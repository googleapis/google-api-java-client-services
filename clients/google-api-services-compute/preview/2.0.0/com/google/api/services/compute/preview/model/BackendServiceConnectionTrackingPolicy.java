...
public final class BackendServiceConnectionTrackingPolicy extends com.google.api.client.json.GenericJson {
   ...
   * connection-oriented protocols (TCP and SCTP) and only if the Tracking Mode isPER_CONNECTION
   ...
   * Balancing](https://cloud.google.com/load-balancing/docs/network/networklb-backend-
   ...
   * seconds).
   *
   * For internal passthrough Network Load Balancers:        - The minimum (default) is 10 minutes
   * and the maximum is 16 hours.    - It can be set only if Connection Tracking is less than
   * 5-tuple    (i.e. Session Affinity is CLIENT_IP_NO_DESTINATION,CLIENT_IP or CLIENT_IP_PROTO, and
   * Tracking    Mode is PER_SESSION).
   *
   * For external passthrough Network Load Balancers the default is 60 seconds. This option is not
   * available publicly.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  ...
  /**
   * Specifies the key used for connection tracking. There are two options:        - PER_CONNECTION:
   * This is the default mode. The Connection    Tracking is performed as per the Connection Key
   * (default Hash Method) for    the specific protocol.    - PER_SESSION: The Connection Tracking
   * is performed as per    the configured Session Affinity. It matches the configured Session
   * Affinity.
   *
   * For more details, see [Tracking Mode for Network Load Balancing](https://cloud.google.com/load-
