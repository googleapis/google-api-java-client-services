/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.cloudbuild.v1beta1.model;

/**
 * Defines the configuration to be used for creating workers in the pool.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the Cloud Build API. For a detailed explanation see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class WorkerConfig extends com.google.api.client.json.GenericJson {

  /**
   * Size of the disk attached to the worker, in GB. See [Worker pool config
   * file](https://cloud.google.com/cloud-build/docs/custom-workers/worker-pool-config-file).
   * Specify a value of up to 1000. If `0` is specified, Cloud Build will use a standard disk size.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key @com.google.api.client.json.JsonString
  private java.lang.Long diskSizeGb;

  /**
   * Machine type of a worker, such as `n1-standard-1`. See [Worker pool config
   * file](https://cloud.google.com/cloud-build/docs/custom-workers/worker-pool-config-file). If
   * left blank, Cloud Build will use `n1-standard-1`.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String machineType;

  /**
   * If true, workers are created without any public address, which prevents network egress to
   * public IPs.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.Boolean noExternalIp;

  /**
   * Size of the disk attached to the worker, in GB. See [Worker pool config
   * file](https://cloud.google.com/cloud-build/docs/custom-workers/worker-pool-config-file).
   * Specify a value of up to 1000. If `0` is specified, Cloud Build will use a standard disk size.
   * @return value or {@code null} for none
   */
  public java.lang.Long getDiskSizeGb() {
    return diskSizeGb;
  }

  /**
   * Size of the disk attached to the worker, in GB. See [Worker pool config
   * file](https://cloud.google.com/cloud-build/docs/custom-workers/worker-pool-config-file).
   * Specify a value of up to 1000. If `0` is specified, Cloud Build will use a standard disk size.
   * @param diskSizeGb diskSizeGb or {@code null} for none
   */
  public WorkerConfig setDiskSizeGb(java.lang.Long diskSizeGb) {
    this.diskSizeGb = diskSizeGb;
    return this;
  }

  /**
   * Machine type of a worker, such as `n1-standard-1`. See [Worker pool config
   * file](https://cloud.google.com/cloud-build/docs/custom-workers/worker-pool-config-file). If
   * left blank, Cloud Build will use `n1-standard-1`.
   * @return value or {@code null} for none
   */
  public java.lang.String getMachineType() {
    return machineType;
  }

  /**
   * Machine type of a worker, such as `n1-standard-1`. See [Worker pool config
   * file](https://cloud.google.com/cloud-build/docs/custom-workers/worker-pool-config-file). If
   * left blank, Cloud Build will use `n1-standard-1`.
   * @param machineType machineType or {@code null} for none
   */
  public WorkerConfig setMachineType(java.lang.String machineType) {
    this.machineType = machineType;
    return this;
  }

  /**
   * If true, workers are created without any public address, which prevents network egress to
   * public IPs.
   * @return value or {@code null} for none
   */
  public java.lang.Boolean getNoExternalIp() {
    return noExternalIp;
  }

  /**
   * If true, workers are created without any public address, which prevents network egress to
   * public IPs.
   * @param noExternalIp noExternalIp or {@code null} for none
   */
  public WorkerConfig setNoExternalIp(java.lang.Boolean noExternalIp) {
    this.noExternalIp = noExternalIp;
    return this;
  }

  @Override
  public WorkerConfig set(String fieldName, Object value) {
    return (WorkerConfig) super.set(fieldName, value);
  }

  @Override
  public WorkerConfig clone() {
    return (WorkerConfig) super.clone();
  }

}
