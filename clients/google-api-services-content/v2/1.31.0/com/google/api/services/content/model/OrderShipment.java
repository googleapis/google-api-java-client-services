...
public final class OrderShipment extends com.google.api.client.json.GenericJson {
   ...
   * "`delhivery`" (Delhivery) - "`dtdc`" (DTDC) - "`tpc india`" (TPC India) Supported carriers for
   * FR are: - "`la poste`" (La Poste) *automatic status updates * - "`colissimo`" (Colissimo by La
   * Poste) *automatic status updates* - "`ups`" (United Parcel Service) *automatic status updates *
   * - "`chronopost`" (Chronopost by La Poste) - "`gls`" (General Logistics Systems France) -
   * "`dpd`" (DPD Group by GeoPost) - "`bpost`" (Belgian Post Group) - "`colis prive`" (Colis Privé)
