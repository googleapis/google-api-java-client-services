...
public final class AccountAdsLink extends com.google.api.client.json.GenericJson {
   ...
   * represents the actual status of the link and can be either `active` if it was approved in
   * Google Ads or `pending` if it's pending approval. Upon insertion, it represents the intended
   * status of the link. Re-uploading a link with status `active` when it's still pending or with
   * status `pending` when it's already active will have no effect: the status will remain
   * unchanged. Re-uploading a link with deprecated status `inactive` is equivalent to not
