 ...
 * system-managed. System-managed keys are also called _Google-owned and managed keys_. User-managed
 * key-pairs can be created and deleted by users. Users are responsible for rotating these keys
 * periodically to ensure security of their service accounts. Users retain the private key of these
 * key-pairs, and Google retains ONLY the public key. System-managed keys that are actively used for
 * signing are rotated regularly according to [security best
 * practices](https://docs.cloud.google.com/iam/docs/key-rotation#timing). The rotation process is
 * probabilistic, and usage of the new key will gradually ramp up and down over the key's lifetime.
 * If you cache the public key set for a service account, we recommend that you update the cache
 * every 15 minutes. User-managed keys can be added and removed at any time, so it is important to
 * update the cache frequently. For Google-managed keys, Google will publish a key at least 6 hours
 * before it is first used for signing and will keep publishing it for at least 6 hours after it was
 * last used for signing. Public keys for all service accounts are also published at the OAuth2
 * Service Account API.
 *
