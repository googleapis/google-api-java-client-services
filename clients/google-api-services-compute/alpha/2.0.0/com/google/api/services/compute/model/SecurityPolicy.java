...
public final class SecurityPolicy extends com.google.api.client.json.GenericJson {
  ...
  /**
   * The type indicates the intended use of the security policy.        - CLOUD_ARMOR: Cloud Armor
   * backend security policies can    be configured to filter incoming HTTP requests targeting
   * backend services.    They filter requests before they hit the origin servers.    -
   * CLOUD_ARMOR_EDGE: Cloud Armor edge security policies can    be configured to filter incoming
   * HTTP requests targeting backend services    (including Cloud CDN-enabled) as well as backend
   * buckets (Cloud Storage).    They filter requests before the request is served from Google's
   * cache.    - CLOUD_ARMOR_INTERNAL_SERVICE (preview only): Cloud Armor    internal service
   * policies can be configured to filter HTTP requests    targeting services managed by Traffic
   * Director in a service mesh. They    filter requests before the request is served from the
   * application.
   *
   * - CLOUD_ARMOR_NETWORK: Cloud Armor network policies can be configured to filter packets
   * targeting network load balancing resources such as backend services, target pools, target
   * instances, and instances with external IPs. They filter requests before the request is served
   ...
   * matching values for these fields.
   *
   * Example:
   *
   *  userDefinedFields:  - name: "ipv4_fragment_offset"    base: IPV4    offset: 6    size: 2
   * mask: "0x1fff"
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * @return value or {@code null} for none
   */
  public SecurityPolicyCloudArmorConfig getCloudArmorConfig() {
    return cloudArmorConfig;
  }

  /**
   * @param cloudArmorConfig cloudArmorConfig or {@code null} for none
   */
  public SecurityPolicy setCloudArmorConfig(SecurityPolicyCloudArmorConfig cloudArmorConfig) {
