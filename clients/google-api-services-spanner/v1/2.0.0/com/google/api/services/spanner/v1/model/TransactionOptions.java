 ...
 * strong read (where Spanner selects a timestamp such that the read is guaranteed to see the
 ...
 * is aborted, the commit fails with error `ABORTED`. If this behavior is undesirable, periodically
 * executing a simple SQL query in the transaction (for example, `SELECT 1`) prevents the
 * transaction from becoming idle. Snapshot read-only transactions: Snapshot read-only transactions
 * provides a simpler method than locking read-write transactions for doing several consistent
 * reads. However, this type of transaction does not support writes. Snapshot transactions do not
 * take locks. Instead, they work by choosing a Cloud Spanner timestamp, then executing all reads at
 * that timestamp. Since they do not acquire locks, they do not block concurrent read-write
 * transactions. Unlike locking read-write transactions, snapshot read-only transactions never
 * abort. They can fail if the chosen read timestamp is garbage collected; however, the default
 * garbage collection policy is generous enough that most applications do not need to worry about
 * this in practice. Snapshot read-only transactions do not need to call Commit or Rollback (and in
 * fact are not permitted to do so). To execute a snapshot transaction, the client specifies a
 * timestamp bound, which tells Cloud Spanner how to choose a read timestamp. The types of timestamp
 * bound are: - Strong (the default). - Bounded staleness. - Exact staleness. If the Cloud Spanner
 * database to be read is geographically distributed, stale read-only transactions can execute more
 * quickly than strong or read-write transactions, because they are able to execute far from the
 * leader replica. Each type of timestamp bound is discussed in detail below. Strong: Strong reads
 * are guaranteed to see the effects of all transactions that have committed before the start of the
 * read. Furthermore, all rows yielded by a single read are consistent with each other -- if any
 * part of the read observes a transaction, all parts of the read see the transaction. Strong reads
 * are not repeatable: two consecutive strong read-only transactions might return inconsistent
 * results if there are concurrent writes. If consistency across reads is required, the reads should
 * be executed within a transaction or at an exact read timestamp. Queries on change streams (see
 ...
 * transactions with a larger commit timestamp. They block until all conflicting transactions that
 * can be assigned commit timestamps <= the read timestamp have finished. The timestamp can either
 * be expressed as an absolute Cloud Spanner commit timestamp or a staleness relative to the current
 * time. These modes do not require a "negotiation phase" to pick a timestamp. As a result, they
 ...
 * replica. Because the timestamp negotiation requires up-front knowledge of which rows are read, it
 ...
 * Spanner can't perform reads at read timestamps more than one hour in the past. This restriction
 ...
 * set to true, a special value of 2^63 - 2 is returned in the Transaction message that describes
 * the transaction, instead of a valid read timestamp. This special value should be discarded and
 * not used for any subsequent queries. Please see https://cloud.google.com/spanner/docs/change-
 * streams for more details on how to query the change stream TVFs. Partitioned DML transactions:
 * Partitioned DML transactions are used to execute DML statements with a different execution
 * strategy that provides different, and often better, scalability properties for large, table-wide
 * operations than DML in a ReadWrite transaction. Smaller scoped statements, such as an OLTP
 * workload, should prefer using ReadWrite transactions. Partitioned DML partitions the keyspace and
 * runs the DML statement on each partition in separate, internal transactions. These transactions
 * commit automatically when complete, and run independently from one another. To reduce lock
 * contention, this execution strategy only acquires read locks on rows that match the WHERE clause
 * of the statement. Additionally, the smaller per-partition transactions hold locks for less time.
 * That said, Partitioned DML is not a drop-in replacement for standard DML used in ReadWrite
 * transactions. - The DML statement must be fully-partitionable. Specifically, the statement must
 * be expressible as the union of many statements which each access only a single row of the table.
 * - The statement is not applied atomically to all rows of the table. Rather, the statement is
 * applied atomically to partitions of the table, in independent transactions. Secondary index rows
 * are updated atomically with the base table rows. - Partitioned DML does not guarantee exactly-
 * once execution semantics against a partition. The statement is applied at least once to each
 * partition. It is strongly recommended that the DML statement should be idempotent to avoid
 * unexpected results. For instance, it is potentially dangerous to run a statement such as `UPDATE
 * table SET column = column + 1` as it could be run multiple times against some rows. - The
 * partitions are committed automatically - there is no support for Commit or Rollback. If the call
 * returns an error, or if the client issuing the ExecuteSql call dies, it is possible that some
 * rows had the statement executed on them successfully. It is also possible that statement was
 * never executed against other rows. - Partitioned DML transactions may only contain the execution
 * of a single DML statement via ExecuteSql or ExecuteStreamingSql. - If any error is encountered
 * during the execution of the partitioned DML operation (for instance, a UNIQUE INDEX violation,
 * division by zero, or a value that can't be stored due to schema constraints), then the operation
 * is stopped at that point and an error is returned. It is possible that at this point, some
 * partitions have been committed (or even committed multiple times), and other partitions have not
 * been run at all. Given the above, Partitioned DML is good fit for large, database-wide,
 * operations that are idempotent, such as deleting old rows from a very large table.
 *
