/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.oracledatabase.v1.model;

/**
 * The Pluggable Database Node Level Details.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the Oracle Database@Google Cloud API. For a detailed
 * explanation see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class PluggableDatabaseNodeLevelDetails extends com.google.api.client.json.GenericJson {

  /**
   * Required. The Node name of the Database home.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String nodeName;

  /**
   * Required. The mode that the pluggable database is in to open it.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String openMode;

  /**
   * Required. The OCID of the Pluggable Database.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String pluggableDatabaseId;

  /**
   * Required. The Node name of the Database home.
   * @return value or {@code null} for none
   */
  public java.lang.String getNodeName() {
    return nodeName;
  }

  /**
   * Required. The Node name of the Database home.
   * @param nodeName nodeName or {@code null} for none
   */
  public PluggableDatabaseNodeLevelDetails setNodeName(java.lang.String nodeName) {
    this.nodeName = nodeName;
    return this;
  }

  /**
   * Required. The mode that the pluggable database is in to open it.
   * @return value or {@code null} for none
   */
  public java.lang.String getOpenMode() {
    return openMode;
  }

  /**
   * Required. The mode that the pluggable database is in to open it.
   * @param openMode openMode or {@code null} for none
   */
  public PluggableDatabaseNodeLevelDetails setOpenMode(java.lang.String openMode) {
    this.openMode = openMode;
    return this;
  }

  /**
   * Required. The OCID of the Pluggable Database.
   * @return value or {@code null} for none
   */
  public java.lang.String getPluggableDatabaseId() {
    return pluggableDatabaseId;
  }

  /**
   * Required. The OCID of the Pluggable Database.
   * @param pluggableDatabaseId pluggableDatabaseId or {@code null} for none
   */
  public PluggableDatabaseNodeLevelDetails setPluggableDatabaseId(java.lang.String pluggableDatabaseId) {
    this.pluggableDatabaseId = pluggableDatabaseId;
    return this;
  }

  @Override
  public PluggableDatabaseNodeLevelDetails set(String fieldName, Object value) {
    return (PluggableDatabaseNodeLevelDetails) super.set(fieldName, value);
  }

  @Override
  public PluggableDatabaseNodeLevelDetails clone() {
    return (PluggableDatabaseNodeLevelDetails) super.clone();
  }

}
