 ...
 * query in the transaction (e.g., `SELECT 1`) prevents the transaction from becoming idle. Snapshot
 * Read-Only Transactions: Snapshot read-only transactions provides a simpler method than locking
 * read-write transactions for doing several consistent reads. However, this type of transaction
 * does not support writes. Snapshot transactions do not take locks. Instead, they work by choosing
 * a Cloud Spanner timestamp, then executing all reads at that timestamp. Since they do not acquire
 * locks, they do not block concurrent read-write transactions. Unlike locking read-write
 * transactions, snapshot read-only transactions never abort. They can fail if the chosen read
 ...
 * bound is discussed in detail below. Strong: Strong reads are guaranteed to see the effects of all
 * transactions that have committed before the start of the read. Furthermore, all rows yielded by a
 * single read are consistent with each other -- if any part of the read observes a transaction, all
 * parts of the read see the transaction. Strong reads are not repeatable: two consecutive strong
 * read-only transactions might return inconsistent results if there are concurrent writes. If
 * consistency across reads is required, the reads should be executed within a transaction or at an
 * exact read timestamp. See TransactionOptions.ReadOnly.strong. Exact Staleness: These timestamp
 * bounds execute reads at a user-specified timestamp. Reads at a timestamp are guaranteed to see a
 * consistent prefix of the global transaction history: they observe modifications done by all
 * transactions with a commit timestamp <= the read timestamp, and observe none of the modifications
 * done by transactions with a larger commit timestamp. They will block until all conflicting
 * transactions that may be assigned commit timestamps <= the read timestamp have finished. The
 * timestamp can either be expressed as an absolute Cloud Spanner commit timestamp or a staleness
 * relative to the current time. These modes do not require a "negotiation phase" to pick a
 * timestamp. As a result, they execute slightly faster than the equivalent boundedly stale
 * concurrency modes. On the other hand, boundedly stale reads usually return fresher results. See
