 ...
 * aborted by Cloud Spanner so that they don't hold on to locks indefinitely. In that case, the
 * commit will fail with error `ABORTED`. If this behavior is undesirable, periodically executing a
 * simple SQL query in the transaction (e.g., `SELECT 1`) prevents the transaction from becoming
 * idle. ## Snapshot Read-Only Transactions Snapshot read-only transactions provides a simpler
 * method than locking read-write transactions for doing several consistent reads. However, this
 * type of transaction does not support writes. Snapshot transactions do not take locks. Instead,
 * they work by choosing a Cloud Spanner timestamp, then executing all reads at that timestamp.
 * Since they do not acquire locks, they do not block concurrent read-write transactions. Unlike
 * locking read-write transactions, snapshot read-only transactions never abort. They can fail if
 * the chosen read timestamp is garbage collected; however, the default garbage collection policy is
 * generous enough that most applications do not need to worry about this in practice. Snapshot
 * read-only transactions do not need to call Commit or Rollback (and in fact are not permitted to
 * do so). To execute a snapshot transaction, the client specifies a timestamp bound, which tells
 * Cloud Spanner how to choose a read timestamp. The types of timestamp bound are: - Strong (the
 * default). - Bounded staleness. - Exact staleness. If the Cloud Spanner database to be read is
 * geographically distributed, stale read-only transactions can execute more quickly than strong or
 * read-write transaction, because they are able to execute far from the leader replica. Each type
 * of timestamp bound is discussed in detail below. ### Strong Strong reads are guaranteed to see
 * the effects of all transactions that have committed before the start of the read. Furthermore,
 * all rows yielded by a single read are consistent with each other -- if any part of the read
 * observes a transaction, all parts of the read see the transaction. Strong reads are not
 * repeatable: two consecutive strong read-only transactions might return inconsistent results if
 * there are concurrent writes. If consistency across reads is required, the reads should be
 ...
 * timestamp <= the read timestamp, and observe none of the modifications done by transactions with
 * a larger commit timestamp. They will block until all conflicting transactions that may be
 * assigned commit timestamps <= the read timestamp have finished. The timestamp can either be
 * expressed as an absolute Cloud Spanner commit timestamp or a staleness relative to the current
 ...
 * Bounded Staleness Bounded staleness modes allow Cloud Spanner to pick the read timestamp, subject
 * to a user-provided staleness bound. Cloud Spanner chooses the newest timestamp within the
 ...
 * TransactionOptions.ReadOnly.max_staleness and TransactionOptions.ReadOnly.min_read_timestamp. ###
 * Old Read Timestamps and Garbage Collection Cloud Spanner continuously garbage collects deleted
 * and overwritten data in the background to reclaim storage space. This process is known as
 * "version GC". By default, version GC reclaims versions after they are one hour old. Because of
 * this, Cloud Spanner cannot perform reads at read timestamps more than one hour in the past. This
 * restriction also applies to in-progress reads and/or SQL queries whose timestamp become too old
 * while executing. Reads and SQL queries with too-old read timestamps fail with the error
 * `FAILED_PRECONDITION`. ## Partitioned DML Transactions Partitioned DML transactions are used to
 * execute DML statements with a different execution strategy that provides different, and often
 * better, scalability properties for large, table-wide operations than DML in a ReadWrite
 * transaction. Smaller scoped statements, such as an OLTP workload, should prefer using ReadWrite
 * transactions. Partitioned DML partitions the keyspace and runs the DML statement on each
 * partition in separate, internal transactions. These transactions commit automatically when
