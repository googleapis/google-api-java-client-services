 ...
 * [Regional](/compute/docs/reference/rest/beta/regionDisks) Persistent disks are required for
...
public final class Disk extends com.google.api.client.json.GenericJson {
   ...
   * customer-supplied key, you must provide the same key if you use the disk later (e.g. to create
   * a disk snapshot, to create a disk image, to create a machine image, or to attach the disk to a
   * virtual machine). Customer-supplied encryption keys do not protect access to metadata of the
   * disk. If you do not provide an encryption key when creating the disk, then the disk will be
