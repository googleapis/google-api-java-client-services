/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.cloudbilling.v1beta.model;

/**
 * VLAN attachment for cloud interconnect.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the Cloud Billing API. For a detailed explanation see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class VlanAttachment extends com.google.api.client.json.GenericJson {

  /**
   * Capacities in the [pricing table](https://cloud.google.com/vpc/network-pricing#interconnect-
   * pricing) Examples of capacity are: 50/100/200/300/400/500-Mbps, 1/2/5/10/20/50-Gbps.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String bandwidth;

  /**
   * VLAN usage. This is specified as a unitless quantity which indicates the number of VLAN
   * attachment used in interconnect.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private Usage vlanCount;

  /**
   * Capacities in the [pricing table](https://cloud.google.com/vpc/network-pricing#interconnect-
   * pricing) Examples of capacity are: 50/100/200/300/400/500-Mbps, 1/2/5/10/20/50-Gbps.
   * @return value or {@code null} for none
   */
  public java.lang.String getBandwidth() {
    return bandwidth;
  }

  /**
   * Capacities in the [pricing table](https://cloud.google.com/vpc/network-pricing#interconnect-
   * pricing) Examples of capacity are: 50/100/200/300/400/500-Mbps, 1/2/5/10/20/50-Gbps.
   * @param bandwidth bandwidth or {@code null} for none
   */
  public VlanAttachment setBandwidth(java.lang.String bandwidth) {
    this.bandwidth = bandwidth;
    return this;
  }

  /**
   * VLAN usage. This is specified as a unitless quantity which indicates the number of VLAN
   * attachment used in interconnect.
   * @return value or {@code null} for none
   */
  public Usage getVlanCount() {
    return vlanCount;
  }

  /**
   * VLAN usage. This is specified as a unitless quantity which indicates the number of VLAN
   * attachment used in interconnect.
   * @param vlanCount vlanCount or {@code null} for none
   */
  public VlanAttachment setVlanCount(Usage vlanCount) {
    this.vlanCount = vlanCount;
    return this;
  }

  @Override
  public VlanAttachment set(String fieldName, Object value) {
    return (VlanAttachment) super.set(fieldName, value);
  }

  @Override
  public VlanAttachment clone() {
    return (VlanAttachment) super.clone();
  }

}
