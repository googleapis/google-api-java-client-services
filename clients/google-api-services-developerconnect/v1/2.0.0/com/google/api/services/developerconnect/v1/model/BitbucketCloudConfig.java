...
public final class BitbucketCloudConfig extends com.google.api.client.json.GenericJson {
  ...
  /**
   * Required. An access token with the minimum `repository`, `pullrequest` and `webhook` scope
   * access. It can either be a workspace, project or repository access token. This is needed to
   * create webhooks. It's recommended to use a system account to generate these credentials.
  ...
  /**
   * Required. An access token with the minimum `repository` access. It can either be a workspace,
   * project or repository access token. It's recommended to use a system account to generate the
   * credentials.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
