/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.texttospeech.v1beta1.model;

/**
 * This contains a mapping between a certain point in the input text and a corresponding time in the
 * output audio.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the Cloud Text-to-Speech API. For a detailed explanation
 * see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class Timepoint extends com.google.api.client.json.GenericJson {

  /**
   * Timepoint name as received from the client within `` tag.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String markName;

  /**
   * Time offset in seconds from the start of the synthesized audio.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.Double timeSeconds;

  /**
   * Timepoint name as received from the client within `` tag.
   * @return value or {@code null} for none
   */
  public java.lang.String getMarkName() {
    return markName;
  }

  /**
   * Timepoint name as received from the client within `` tag.
   * @param markName markName or {@code null} for none
   */
  public Timepoint setMarkName(java.lang.String markName) {
    this.markName = markName;
    return this;
  }

  /**
   * Time offset in seconds from the start of the synthesized audio.
   * @return value or {@code null} for none
   */
  public java.lang.Double getTimeSeconds() {
    return timeSeconds;
  }

  /**
   * Time offset in seconds from the start of the synthesized audio.
   * @param timeSeconds timeSeconds or {@code null} for none
   */
  public Timepoint setTimeSeconds(java.lang.Double timeSeconds) {
    this.timeSeconds = timeSeconds;
    return this;
  }

  @Override
  public Timepoint set(String fieldName, Object value) {
    return (Timepoint) super.set(fieldName, value);
  }

  @Override
  public Timepoint clone() {
    return (Timepoint) super.clone();
  }

}
