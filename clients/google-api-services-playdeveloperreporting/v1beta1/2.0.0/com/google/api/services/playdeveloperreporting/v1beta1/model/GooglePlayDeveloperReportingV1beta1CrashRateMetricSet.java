 ...
 * period that experienced at least one crash. * `crashRate7dUserWeighted` (`google.type.Decimal`):
 * Rolling average value of `crashRate` in the last 7 days. The daily values are weighted by the
 * count of distinct users for the day. * `crashRate28dUserWeighted` (`google.type.Decimal`):
 * Rolling average value of `crashRate` in the last 28 days. The daily values are weighted by the
 * count of distinct users for the day. * `userPerceivedCrashRate` (`google.type.Decimal`):
 ...
 * distinct users for the day. * `userPerceivedCrashRate28dUserWeighted` (`google.type.Decimal`):
 * Rolling average value of `userPerceivedCrashRate` in the last 28 days. The daily values are
