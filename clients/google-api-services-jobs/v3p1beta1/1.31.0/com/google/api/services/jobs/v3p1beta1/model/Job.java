...
public final class Job extends com.google.api.client.json.GenericJson {
   ...
   * employer is looking to hire for this job posting. Specifying the full street address(es) of the
   * hiring location enables better API results, especially job searches by commute time. At most 50
   * locations are allowed for best search performance. If a job has more locations, it is suggested
   * to split it into multiple jobs with unique requisition_ids (e.g. 'ReqA' becomes 'ReqA-1',
   * 'ReqA-2', etc.) as multiple jobs with the same company_name, language_code and requisition_id
   * are not allowed. If the original requisition_id must be preserved, a custom field should be
   * used for storage. It is also suggested to group the locations that close to each other in the
   * same job for better search experience. Jobs with multiple addresses must have their addresses
   * with the same LocationType to allow location filtering to work properly. (For example, a Job
   * with addresses "1600 Amphitheatre Parkway, Mountain View, CA, USA" and "London, UK" may not
   * have location filters applied correctly at search time since the first is a
   * LocationType.STREET_ADDRESS and the second is a LocationType.LOCALITY.) If a job needs to have
   * multiple addresses, it is suggested to split it into multiple jobs with same LocationTypes. The
  ...
  /**
   * Required. At least one field within ApplicationInfo must be specified. Job application
   * information.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * are not covered by the provided structured fields. The keys of the map are strings up to 64
   * bytes and must match the pattern: a-zA-Z*. For example, key0LikeThis or KEY_1_LIKE_THIS. At
   * most 100 filterable and at most 100 unfilterable keys are supported. For filterable
   ...
   * when a job is created. The format is "projects/{project_id}/jobs/{job_id}", for example,
   ...
   * if there is another existing open job with same company_name, language_code and requisition_id.
   ...
   * cleaned out. Invalid timestamps are ignored, and treated as expire time not provided. Timestamp
   * before the instant request is made is considered valid, the job will be treated as expired
   * immediately. If this value is not provided at the time of job creation or is invalid, the job
   * posting expires after 30 days from the job's creation time. For example, if the job was created
   * on 2017/01/01 13:00AM UTC with an unspecified expiration date, the job expires after 2017/01/31
   * 13:00AM UTC. If this value is not provided on job update, it depends on the field masks set by
   ...
   * [company_name], language_code and requisition_id. The maximum number of allowed characters is
   * 255.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
