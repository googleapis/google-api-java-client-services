...
public class Drive extends com.google.api.client.googleapis.services.json.AbstractGoogleJsonClient {
  ...
  public class Files {
     ...
     * only works if the file is stored in Drive. To download Google Docs, Sheets, and Slides use [`file
     * s.export`](https://developers.google.com/workspace/drive/api/reference/rest/v2/files/export)
    ...
    public class Get extends DriveRequest<com.google.api.services.drive.model.File> {
       ...
       * Gets a file's metadata or content by ID. If you provide the URL parameter `alt=media`, then the
       * response includes the file contents in the response body. Downloading content with `alt=media`
       * only works if the file is stored in Drive. To download Google Docs, Sheets, and Slides use [`fi
       * les.export`](https://developers.google.com/workspace/drive/api/reference/rest/v2/files/export)
       * instead. For more information, see [Download & export
       * files](https://developers.google.com/workspace/drive/api/guides/manage-downloads).
       *
       * Create a request for the method "files.get".
     ...
     * see [Upload file data](https://developers.google.com/workspace/drive/api/guides/manage-uploads).
     * Apps creating shortcuts with `files.insert` must specify the MIME type `application/vnd.google-
     * apps.shortcut`. Apps should specify a file extension in the `title` property when inserting files
     * with the API. For example, an operation to insert a JPEG file should specify something like
     * `"title": "cat.jpg"` in the metadata. Subsequent `GET` requests include the read-only
    ...
    public class Insert extends DriveRequest<com.google.api.services.drive.model.File> {
       ...
       * files, see [Upload file data](https://developers.google.com/workspace/drive/api/guides/manage-
       * uploads). Apps creating shortcuts with `files.insert` must specify the MIME type
       * `application/vnd.google-apps.shortcut`. Apps should specify a file extension in the `title`
       * property when inserting files with the API. For example, an operation to insert a JPEG file
       * should specify something like `"title": "cat.jpg"` in the metadata. Subsequent `GET` requests
       * include the read-only `fileExtension` property populated with the extension originally
       * specified in the `title` property. When a Google Drive user requests to download a file, or
       * when the file is downloaded through the sync client, Drive builds a full filename (with
       * extension) based on the title. In cases where the extension is missing, Drive attempts to
  ...
  public class Permissions {
    ...
    public class Insert extends DriveRequest<com.google.api.services.drive.model.Permission> {
      ...
      /** Deprecated: All requests use the expansive access rules. */
      public Insert setEnforceExpansiveAccess(java.lang.Boolean enforceExpansiveAccess) {
        this.enforceExpansiveAccess = enforceExpansiveAccess;
        return this;
      }
