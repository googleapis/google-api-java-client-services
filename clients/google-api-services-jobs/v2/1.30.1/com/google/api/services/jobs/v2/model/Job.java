...
public final class Job extends com.google.api.client.json.GenericJson {
   ...
   * application_instruction must be specified. Use this field to specify email address(es) to which
   * resumes or applications can be sent. The maximum number of allowed characters is 255.
  ...
  /**
   * Optional but at least one of application_urls, application_email_list or
   * application_instruction must be specified. Use this field to provide instructions, such as
   * "Mail your application to ...", that a candidate can follow to apply for the job. This field
   * accepts and sanitizes HTML input, and also accepts bold, italic, ordered list, and unordered
   * list markup tags. The maximum number of allowed characters is 3,000.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  ...
  /**
   * Optional but at least one of application_urls, application_email_list or
   * application_instruction must be specified. Use this URL field to direct an applicant to a
   * website, for example to link to an online application form. The maximum number of allowed
   * characters is 2,000.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  ...
  /**
   * Optional but one of company_name or distributor_company_id must be provided. The resource name
   * of the company listing the job, such as /companies/foo. This field takes precedence over the
  ...
  /**
   * Deprecated. Use company_display_name instead. Output only. The name of the company listing the
   * job.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * are not covered by the provided structured fields. This field is a more general combination of
   * the deprecated id-based filterable_custom_fields and string-based non_filterable_custom_fields.
   * The keys of the map are strings up to 64 bytes and must match the pattern: a-zA-Z*. At most 100
  ...
  /**
   * Optional. The department or functional area within the company with the open position. The
  ...
  /**
   * Optional but one of company_name or distributor_company_id must be provided. A unique company
   * identifier used by job distributors to identify an employer's company entity. company_name
  ...
  /**
   * Optional but strongly recommended for the best service experience. The expiration timestamp of
   * the job. After this timestamp, the job is marked as expired, and it no longer appears in search
   * results. The expired job can't be deleted or listed by the DeleteJob and ListJobs APIs, but it
   * can be retrieved with the GetJob API or updated with the UpdateJob API. An expired job can be
   * updated and opened again by using a future expiration timestamp. Updating an expired job fails
   * if there is another existing open job with same requisition_id, company_name and language_code.
   ...
   * [https://www.ietf.org/rfc/rfc3339.txt](https://www.ietf.org/rfc/rfc3339.txt). A valid date
   * range is between 1970-01-01T00:00:00.0Z and 2100-12-31T23:59:59.999Z. Invalid dates are ignored
   * and treated as expire time not provided. If this value is not provided at the time of job
   * creation or is invalid, the job posting expires after 30 days from the job's creation time. For
   * example, if the job was created on 2017/01/01 13:00AM UTC with an unspecified expiration date,
   * the job expires after 2017/01/31 13:00AM UTC. If this value is not provided but expiry_date is,
   * expiry_date is used. If this value is not provided on job update, it depends on the field masks
   * set by UpdateJobRequest.update_job_fields. If the field masks include expiry_time, or the masks
   * are empty meaning that every field is updated, the job posting expires after 30 days from the
  ...
  /**
   * Deprecated. Use expire_time instead. Optional but strongly recommended to be provided for the
   * best service experience. The expiration date of the job in UTC time. After 12 am on this date,
   * the job is marked as expired, and it no longer appears in search results. The expired job can't
   * be deleted or listed by the DeleteJob and ListJobs APIs, but it can be retrieved with the
   * GetJob API or updated with the UpdateJob API. An expired job can be updated and opened again by
   * using a future expiration date. It can also remain expired. Updating an expired job to be open
   * fails if there is another existing open job with same requisition_id, company_name and
   * language_code. The expired jobs are retained in our system for 90 days. However, the overall
   * expired job count cannot exceed 3 times the maximum of open jobs count over the past week,
   * otherwise jobs with earlier expire time are removed first. Expired jobs are no longer
   * accessible after they are cleaned out. A valid date range is between 1970/1/1 and 2100/12/31.
   * Invalid dates are ignored and treated as expiry date not provided. If this value is not
   * provided on job creation or is invalid, the job posting expires after 30 days from the job's
   * creation time. For example, if the job was created on 2017/01/01 13:00AM UTC with an
   * unspecified expiration date, the job expires after 2017/01/31 13:00AM UTC. If this value is not
  ...
  /**
   * Deprecated. Always use compensation_info. Optional. Job compensation information. This field
   * replaces compensation_info. Only CompensationInfo.entries or extended_compensation_info can be
   * set, otherwise an exception is thrown.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  ...
  /**
   * Deprecated. Use custom_attributes instead. Optional. A map of fields to hold filterable custom
   * job attributes not captured by the standard fields such as job_title, company_name, or level.
   * These custom fields store arbitrary string values, and can be used for purposes not covered by
   * the structured fields. For the best search experience, use of the structured rather than custom
   * fields is recommended. Data stored in these custom fields fields are indexed and searched
   * against by keyword searches (see SearchJobsRequest.custom_field_filters][]). The map key must
   * be a number between 1-20. If an invalid key is provided on job create or update, an error is
   * returned.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * the job not including salary or pay. The maximum number of allowed characters is 10,000.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * that are associated with the job. Language codes must be in BCP-47 format, such as "en-US" or
   * "sr-Latn". For more information, see [Tags for Identifying
   * Languages](https://tools.ietf.org/html/bcp47){: class="external" target="_blank" }. If this
   * field is unspecified and Job.description is present, detected language code based on
  ...
  /**
   * Optional but strongly recommended for the best service experience. Location(s) where the
   * emploeyer is looking to hire for this job posting. Specifying the full street address(es) of
   * the hiring location enables better API results, especially job searches by commute time. At
   * most 50 locations are allowed for best search performance. If a job has more locations, it is
   * suggested to split it into multiple jobs with unique requisition_ids (e.g. 'ReqA' becomes
   ...
   * other in the same job for better search experience. The maximum number of allowed characters is
   * 500.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * Only the jobs with a promotionValue >0 are returned in a FEATURED_JOB_SEARCH. Default value is
   ...
   * field is recommended as an alternative to using the more general description field. This field
   * accepts and sanitizes HTML input, and also accepts bold, italic, ordered list, and unordered
   ...
   * requisition_id, company_name and language_code. The maximum number of allowed characters is
   * 255.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * alternative to using the more general description field. This field accepts and sanitizes HTML
   * input, and also accepts bold, italic, ordered list, and unordered list markup tags. The maximum
  ...
  /**
   * Deprecated. Use custom_attributes instead. Optional. A map of fields to hold non-filterable
   * custom job attributes, similar to filterable_custom_fields. These fields are distinct in that
   * the data in these fields are not indexed. Therefore, the client cannot search against them, nor
   * can the client use them to list jobs. The key of the map can be any valid string.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
