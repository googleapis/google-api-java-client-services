 ...
 * The following load balancer can use either regional or global health check: * Internal TCP/UDP
 * load balancer The following load balancers require regional health check: * Internal HTTP(S) load
 * balancer * Backend service-based network load balancer Traffic Director and the following load
 * balancers require global health check: * External HTTP(S) load balancer * TCP proxy load balancer
 * * SSL proxy load balancer The following load balancer require [legacy HTTP health
 * checks](/compute/docs/reference/rest/v1/httpHealthChecks): * Target pool-based network load
 * balancer **Autohealing in MIGs** The health checks that you use for autohealing VMs in a MIG can
 * be either regional or global. For more information, see Set up an application health check and
