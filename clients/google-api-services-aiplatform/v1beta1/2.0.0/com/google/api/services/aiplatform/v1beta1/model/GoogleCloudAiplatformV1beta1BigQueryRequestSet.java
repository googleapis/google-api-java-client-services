/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://github.com/googleapis/google-api-java-client-services/
 * Modify at your own risk.
 */

package com.google.api.services.aiplatform.v1beta1.model;

/**
 * The request set for the evaluation run.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the Vertex AI API. For a detailed explanation see:
 * <a href="https://developers.google.com/api-client-library/java/google-http-java-client/json">https://developers.google.com/api-client-library/java/google-http-java-client/json</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class GoogleCloudAiplatformV1beta1BigQueryRequestSet extends com.google.api.client.json.GenericJson {

  /**
   * Optional. Map of candidate name to candidate response column name. The column will be in
   * evaluation_item.CandidateResponse format.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.util.Map<String, java.lang.String> candidateResponseColumns;

  /**
   * Optional. The name of the column that contains the requests to evaluate. This will be in
   * evaluation_item.EvalPrompt format.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String promptColumn;

  /**
   * Optional. The name of the column that contains the rubrics. This is in
   * evaluation_rubric.RubricGroup format.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String rubricsColumn;

  /**
   * Optional. The sampling config for the bigquery resource.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private GoogleCloudAiplatformV1beta1BigQueryRequestSetSamplingConfig samplingConfig;

  /**
   * Required. The URI of a BigQuery table. e.g. bq://projectId.bqDatasetId.bqTableId
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String uri;

  /**
   * Optional. Map of candidate name to candidate response column name. The column will be in
   * evaluation_item.CandidateResponse format.
   * @return value or {@code null} for none
   */
  public java.util.Map<String, java.lang.String> getCandidateResponseColumns() {
    return candidateResponseColumns;
  }

  /**
   * Optional. Map of candidate name to candidate response column name. The column will be in
   * evaluation_item.CandidateResponse format.
   * @param candidateResponseColumns candidateResponseColumns or {@code null} for none
   */
  public GoogleCloudAiplatformV1beta1BigQueryRequestSet setCandidateResponseColumns(java.util.Map<String, java.lang.String> candidateResponseColumns) {
    this.candidateResponseColumns = candidateResponseColumns;
    return this;
  }

  /**
   * Optional. The name of the column that contains the requests to evaluate. This will be in
   * evaluation_item.EvalPrompt format.
   * @return value or {@code null} for none
   */
  public java.lang.String getPromptColumn() {
    return promptColumn;
  }

  /**
   * Optional. The name of the column that contains the requests to evaluate. This will be in
   * evaluation_item.EvalPrompt format.
   * @param promptColumn promptColumn or {@code null} for none
   */
  public GoogleCloudAiplatformV1beta1BigQueryRequestSet setPromptColumn(java.lang.String promptColumn) {
    this.promptColumn = promptColumn;
    return this;
  }

  /**
   * Optional. The name of the column that contains the rubrics. This is in
   * evaluation_rubric.RubricGroup format.
   * @return value or {@code null} for none
   */
  public java.lang.String getRubricsColumn() {
    return rubricsColumn;
  }

  /**
   * Optional. The name of the column that contains the rubrics. This is in
   * evaluation_rubric.RubricGroup format.
   * @param rubricsColumn rubricsColumn or {@code null} for none
   */
  public GoogleCloudAiplatformV1beta1BigQueryRequestSet setRubricsColumn(java.lang.String rubricsColumn) {
    this.rubricsColumn = rubricsColumn;
    return this;
  }

  /**
   * Optional. The sampling config for the bigquery resource.
   * @return value or {@code null} for none
   */
  public GoogleCloudAiplatformV1beta1BigQueryRequestSetSamplingConfig getSamplingConfig() {
    return samplingConfig;
  }

  /**
   * Optional. The sampling config for the bigquery resource.
   * @param samplingConfig samplingConfig or {@code null} for none
   */
  public GoogleCloudAiplatformV1beta1BigQueryRequestSet setSamplingConfig(GoogleCloudAiplatformV1beta1BigQueryRequestSetSamplingConfig samplingConfig) {
    this.samplingConfig = samplingConfig;
    return this;
  }

  /**
   * Required. The URI of a BigQuery table. e.g. bq://projectId.bqDatasetId.bqTableId
   * @return value or {@code null} for none
   */
  public java.lang.String getUri() {
    return uri;
  }

  /**
   * Required. The URI of a BigQuery table. e.g. bq://projectId.bqDatasetId.bqTableId
   * @param uri uri or {@code null} for none
   */
  public GoogleCloudAiplatformV1beta1BigQueryRequestSet setUri(java.lang.String uri) {
    this.uri = uri;
    return this;
  }

  @Override
  public GoogleCloudAiplatformV1beta1BigQueryRequestSet set(String fieldName, Object value) {
    return (GoogleCloudAiplatformV1beta1BigQueryRequestSet) super.set(fieldName, value);
  }

  @Override
  public GoogleCloudAiplatformV1beta1BigQueryRequestSet clone() {
    return (GoogleCloudAiplatformV1beta1BigQueryRequestSet) super.clone();
  }

}
