 ...
 *
 * This response is validated by the Dialogflow server. If validation fails, an error will be
 * returned in the QueryResult.diagnostic_info field. Setting JSON fields to an empty value with the
 * wrong type is a common error. To avoid this error:
 *
 * - Use `""` for empty strings - Use `{}` or `null` for empty objects - Use `[]` or `null` for
 * empty arrays
 *
 * For more information, see the [Protocol Buffers Language Guide](https://developers.google.com
...
public final class GoogleCloudDialogflowV2beta1WebhookResponse extends com.google.api.client.json.GenericJson {
   ...
   * entity synonyms apply to all languages and persist for the session of this query. Setting the
   * session entity types inside webhook overwrites the session entity types that have been set
   * through `DetectIntentRequest.query_params.session_entity_types`.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
