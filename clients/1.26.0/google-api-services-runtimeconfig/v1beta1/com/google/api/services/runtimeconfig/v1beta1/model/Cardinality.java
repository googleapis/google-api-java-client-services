 ...
 * Cardinality condition where the `path` is set to `/foo` and the number of paths is set to 2, the
 * following variables would meet the condition in a RuntimeConfig resource:
 *
 * + `/foo/variable1 = "value1"` + `/foo/variable2 = "value2"` + `/bar/variable3 = "value3"`
 *
 * It would not would not satisify the same condition with the `number` set to 3, however, because
 * there is only 2 paths that start with `/foo`. Cardinality conditions are recursive; all subtrees
 * under the specific path prefix are counted.
 *
