...
/**
 * HTTP request. The task will be pushed to the worker as an HTTP request. An HTTP request embodies
 * a url, an http method, headers, body and authorization for the http task.
 *
...
public final class HttpRequest extends com.google.api.client.json.GenericJson {
   ...
   * when running the task is created or task is created. These headers represent a subset of the
   * headers that will accompany the task's HTTP request. Some HTTP request headers will be ignored
   * or replaced. A partial list of headers that will be ignored or replaced is: * Any header that
   * is prefixed with "X-CloudTasks-" will be treated as service header. Service headers define
   * properties of the task and are predefined in CloudTask. * Host: This will be computed by Cloud
   * Tasks and derived from HttpRequest.url. * Content-Length: This will be computed by Cloud Tasks.
