 ...
 * to a single `role`. Principals can be user accounts, service accounts, Google groups, and domains
 * (such as G Suite). A `role` is a named list of permissions; each `role` can be an IAM predefined
...
public final class Policy extends com.google.api.client.json.GenericJson {
   ...
   *
   * The `bindings` in a `Policy` can refer to up to 1,500 principals; up to 250 of these principals
   * can be Google groups. Each occurrence of a principal counts towards these limits. For example,
   * if the `bindings` grant 50 different roles to `user:alice@example.com`, and not to any other
   * principal, then you can add another 1,450 principals to the `bindings` in the `Policy`.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
