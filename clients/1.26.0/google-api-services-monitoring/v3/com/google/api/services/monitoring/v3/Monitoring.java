...
public class Monitoring extends com.google.api.client.googleapis.services.json.AbstractGoogleJsonClient {
  ...
  public class MetricDescriptors {
    ...
    public class Delete extends MonitoringRequest<com.google.api.services.monitoring.v3.model.Empty> {
      ...
      private final java.util.regex.Pattern NAME_PATTERN =
          java.util.regex.Pattern.compile("^[^/]+/[^/]+/metricDescriptors/.+$");
  ...
  public class MonitoredResourceDescriptors {
    ...
    public class Get extends MonitoringRequest<com.google.api.services.monitoring.v3.model.MonitoredResourceDescriptor> {
      ...
      private final java.util.regex.Pattern NAME_PATTERN =
          java.util.regex.Pattern.compile("^[^/]+/[^/]+/monitoredResourceDescriptors/[^/]+$");
    ...
    public class List extends MonitoringRequest<com.google.api.services.monitoring.v3.model.ListMonitoredResourceDescriptorsResponse> {
       ...
       * descriptor's type and labels. For example, the following filter returns only Google Compute
       * Engine descriptors that have an id label: resource.type = starts_with("gce_") AND
       * resource.label:id
       */
      @com.google.api.client.util.Key
      private java.lang.String filter;
  ...
  public class TimeSeries {
    ...
    public class List extends MonitoringRequest<com.google.api.services.monitoring.v3.model.ListTimeSeriesResponse> {
       ...
       * least 60 seconds. After per-time series alignment, each time series will contain data
       * points only on the period boundaries. If perSeriesAligner is not specified or equals
       * ALIGN_NONE, then this field is ignored. If perSeriesAligner is specified and does not equal
     ...
     defined; otherwise an error is returned.
      ...
      /**
       * The approach to be used to combine time series. Not all reducer functions may be applied to
       * all time series, depending on the metric type and the value type of the original time
       * series. Reduction may change the metric type of value type of the time series.Time series
       * data must be aligned in order to perform cross-time series reduction. If crossSeriesReducer
       * is specified, then perSeriesAligner must be specified and not equal ALIGN_NONE and
     ...
     otherwise, an error is returned.
       ...
       * The set of fields to preserve when crossSeriesReducer is specified. The groupByFields
       * determine how the time series are partitioned into subsets prior to applying the
       * aggregation function. Each subset contains time series that have the same value for each of
       * the grouping fields. Each individual time series is a member of exactly one subset. The
       * crossSeriesReducer is applied to each subset of time series. It is not possible to reduce
       * across different resource types, so this field implicitly contains resource.type. Fields
       * not specified in groupByFields are aggregated away. If groupByFields is not specified and
       * all the time series have the same resource type, then the time series are aggregated into a
       * single output time series. If crossSeriesReducer is not defined, this field is ignored.
       */
      @com.google.api.client.util.Key("aggregation.groupByFields")
      private java.util.List<java.lang.String> aggregationGroupByFields;
     ...
     field is ignored.
       ...
       * applied to all time series, depending on the metric type and value type of the original
       * time series. Alignment may change the metric type or the value type of the time series.Time
       * series data must be aligned in order to perform cross-time series reduction. If
       * crossSeriesReducer is specified, then perSeriesAligner must be specified and not equal
       * ALIGN_NONE and alignmentPeriod must be specified; otherwise, an error is returned.
       */
      @com.google.api.client.util.Key("aggregation.perSeriesAligner")
     ...
     The start time must not be later than the end time.
      ...
      /**
       * A positive number that is the maximum number of results to return. If page_size is empty or
       * more than 100,000 results, the effective page_size is 100,000 results. If view is set to
       * FULL, this is the maximum number of Points returned. If view is set to HEADERS, this is the
