 ...
 * task will be sent to is: * `url =` host `+` relative_url Tasks can be dispatched to secure app
...
public final class AppEngineHttpRequest extends com.google.api.client.json.GenericJson {
   ...
   * to the modified `User-Agent`. If the task has a payload, Cloud Tasks sets the following
   ...
   * `"application/json"`. * `Content-Length`: This is computed by Cloud Tasks. This value is output
   * only. It cannot be changed. The headers below cannot be set or overridden: * `Host` *
   * `X-Google-*` * `X-AppEngine-*` In addition, Cloud Tasks sets some headers when the task is
   * dispatched, such as headers containing information about the task; see [request
  ...
  /**
   * Payload. The payload will be sent as the HTTP message body. A message body, and thus a payload,
   * is allowed only if the HTTP method is POST or PUT. It is an error to set a data payload on a
   * task with an incompatible HttpMethod.
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
   ...
   * It can contain a path and query string arguments. If the relative URL is empty, then the root
