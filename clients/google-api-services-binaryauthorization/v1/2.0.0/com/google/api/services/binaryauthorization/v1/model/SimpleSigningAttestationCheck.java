...
public final class SimpleSigningAttestationCheck extends com.google.api.client.json.GenericJson {
  ...
  /**
   * Optional. The projects where attestations are stored as Container Analysis Occurrences. Only
   * one attestation needs to successfully verify an image for this check to pass, so a single
   * verified attestation found in any of `container_analysis_attestation_projects` is sufficient
   * for the check to pass. When fetching Occurrences from Container Analysis, only
