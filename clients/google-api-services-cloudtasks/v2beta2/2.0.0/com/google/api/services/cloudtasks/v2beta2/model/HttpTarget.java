...
public final class HttpTarget extends com.google.api.client.json.GenericJson {
   ...
   * when running the task is created and/or task is created. These headers represent a subset of
   * the headers that will accompany the task's HTTP request. Some HTTP request headers will be
   * ignored or replaced. A partial list of headers that will be ignored or replaced is: * Any
   * header that is prefixed with "X-CloudTasks-" will be treated as service header. Service headers
   * define properties of the task and are predefined in CloudTask. * Host: This will be computed by
   * Cloud Tasks and derived from HttpRequest.url. * Content-Length: This will be computed by Cloud
   * Tasks. * User-Agent: This will be set to `"Google-CloudTasks"`. * `X-Google-*`: Google use
   * only. * `X-AppEngine-*`: Google use only. `Content-Type` won't be set by Cloud Tasks. You can
   ...
   * size of the headers must be less than 80KB. Queue-level headers to override headers of all the
   * tasks in the queue. Do not put business sensitive or personally identifying data in the HTTP
   * Header Override Configuration or other similar fields in accordance with Section 12 (Resource
   * Fields) of the [Service Specific Terms](https://cloud.google.com/terms/service-terms).
