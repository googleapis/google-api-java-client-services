...
public final class GoogleFirestoreAdminV1Field extends com.google.api.client.json.GenericJson {
   ...
   * ction_id}/fields/{field_path}` A field path may be a simple field name, e.g. `address` or a
   * path to fields within map_value , e.g. `address.city`, or a special field path. The only valid
   * special field is `*`, which represents any field. Field paths may be quoted using ` (backtick).
   * The only character that needs to be escaped within a quoted field path is the backtick
   * character itself, escaped using a backslash. Special characters in field paths that must be
   * quoted include: `*`, `.`, ``` (backtick), `[`, `]`, as well as any ascii symbolic characters.
   ...
   * `*`, not any field. A special `Field` contains the default indexing settings for all fields.
