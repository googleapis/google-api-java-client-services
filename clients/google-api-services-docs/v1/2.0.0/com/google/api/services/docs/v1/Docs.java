...
public class Docs extends com.google.api.client.googleapis.services.json.AbstractGoogleJsonClient {
  ...
  public class Documents {
    ...
    public class Get extends DocsRequest<com.google.api.services.docs.v1.model.Document> {
       ...
       * document content. `Document.tabs` will be empty. If you use a field mask that references
       * the `Document.tabs` field (or any subfield), the API implicitly treats the request as if
       * you set `include_tabs_content` to `true`.
       */
      @com.google.api.client.util.Key
     ...
     `documentStyle` on `Document`. - When `true`: Document content populates in the `Document.tabs`
     field instead of the text content fields in `Document`. - When `false`: The content of the
     document's first tab populates the content fields in `Document` excluding `Document.tabs`. If a
     document has only one tab, then that tab is used to populate the document content. `Document.tabs`
     will be empty. If you use a field mask that references the `Document.tabs` field (or any subfield),
     the API implicitly treats the request as if you set `include_tabs_content` to `true`.
       */
